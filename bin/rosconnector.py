    ...
    def step(self, msg): 
        try:
            ...
            if not rospy.is_shutdown():
                ...
                if self.is_terminated or self.in_reset or self.is_crash or self.is_goal_reached:
                    rospy.loginfo('Discard action terminate:'+ str(self.is_terminated) + 'crach:' + str(self.is_crash))
                    self.print_if_verbose('Discard action because turtleBot is in reset or terminated or crashed or goal reached')
                    stopTurtleBot(self.__set_navigation_publisher)
                    self.__crash = False
                    self.__turtleBot_in_position = False
                    self.__first_action_taken = False
                    if self.is_terminated or self.is_crash:
                        self.__terminated = True
                        self.__crash = True
                        self.__terminate_publisher.publish(Bool(True)) 
                    return
                else:
                    if not self.__turtleBot_in_position:
                        self.positionResetter()
                        ...
                        odomMsg = rospy.wait_for_message('/odom', Odometry)
                        [x_pos, y_pos]= getPosition(odomMsg)
                        self.__goal_reached = checkGoalNear(x_pos, y_pos, self.X_GOAL, self.Y_GOAL)
                        if not self.is_crash and not self.is_goal_reached:
