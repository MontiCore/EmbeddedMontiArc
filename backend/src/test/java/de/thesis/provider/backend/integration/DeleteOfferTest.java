...
public class DeleteOfferTest {
	...
	@Test
	public void deleteOneOffer() throws Exception {
		...
		// First, offer is created so that this test does not destory the initial data loaded into the system
		...
		// After a new offer was created, it can safely be deleted without removing other data
		...
		mvc.perform(get("/offers")
				...
				.andExpect(status().isOk())
				// After deleting the new offer, there should be 3 offers left
