...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, **kwargs):
        ...
        with self.name_scope():
            ...
            lastEpoch = 0
            for file in os.listdir("src/main/resources/simplePretrained"):
                if "simple_embedding-" in file and ".json" in file:
                    symbolFile = file

                if "simple_embedding-" in file and ".param" in file:
                    epochStr = file.replace(".params", "").replace("simple_embedding-", "")
                    epoch = int(epochStr)
                    if epoch >= lastEpoch:
                        lastEpoch = epoch
                        weightFile = file
            ...
            inputNames = []
            zeroInputs = []
            inputNames.append("data" + str(0))
            zeroInputs.append(nd.zeros((1,5), ctx=mx_context[0]))
            inputNames.append("data" + str(1))
            zeroInputs.append(nd.zeros((1,5), ctx=mx_context[0]))
            inputNames.append("data" + str(2))
            zeroInputs.append(nd.zeros((1,1), ctx=mx_context[0]))
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                self.loadnetwork4_1_ = gluon.nn.SymbolBlock.imports("src/main/resources/simplePretrained/" + symbolFile, inputNames, "src/main/resources/simplePretrained/" + weightFile, ctx=mx_context)
            self.loadnetwork4_1_out_shape = self.loadnetwork4_1_(*zeroInputs).shape
            if self.loadnetwork4_1_out_shape != (1,5,768):
                outputSize=1
                for x in (5,768,): 
                    outputSize = outputSize * x  
                self.loadnetwork4_1_fc_ = gluon.nn.Dense(units=outputSize, use_bias=False, flatten=False)

            self.dotproductselfattention8_1_ = DotProductSelfAttention(scale_factor=-1, num_heads=16, dim_model=768, dim_keys=-1, dim_values=-1, use_proj_bias=True, use_mask=False)
            # dotproductselfattention8_1_, output shape: {[5,768,1]}

            self.dropout8_1_ = gluon.nn.Dropout(rate=0.1)
            self.layernorm9_ = gluon.nn.LayerNorm()
            # layernorm9_, output shape: {[5,768,1]}

            self.memory9_ = LargeMemory(sub_key_size=512, query_size=[512], query_act="linear", 
                                               k=32, num_heads=4, values_dim=-1)
            self.layernorm10_ = gluon.nn.LayerNorm()
            # layernorm10_, output shape: {[1,512,1]}

            self.fc10_ = gluon.nn.Dense(units=768, use_bias=True, flatten=True)
            # fc10_, output shape: {[768,1,1]}

            self.dropout10_ = gluon.nn.Dropout(rate=0.1)
            self.fc11_ = gluon.nn.Dense(units=33, use_bias=True, flatten=True)
            # fc11_, output shape: {[33,1,1]}
    ...
    def hybrid_forward(self, F, data_0_, data_1_, data_2_):
        ...
        loadnetwork4_1_ = self.loadnetwork4_1_(data_0_,data_1_,data_2_)
        if self.loadnetwork4_1_out_shape != (1,5,768):
            loadnetwork4_1_ = self.loadnetwork4_1_fc_(loadnetwork4_1_)
            loadnetwork4_1_ = F.reshape(loadnetwork4_1_, shape=(-1,5,768))
        get4_2_ = data_2_
        get7_1_1_ = loadnetwork4_1_
        get7_1_2_ = loadnetwork4_1_
        get7_1_3_ = loadnetwork4_1_
        get7_1_4_ = get4_2_
        dotproductselfattention8_1_ = self.dotproductselfattention8_1_(get7_1_1_, get7_1_2_, get7_1_3_, get7_1_4_)
        dropout8_1_ = self.dropout8_1_(dotproductselfattention8_1_)
        get6_2_ = loadnetwork4_1_
        add9_ = dropout8_1_ + get6_2_
        layernorm9_ = self.layernorm9_(add9_)
        memory9_ = self.memory9_(layernorm9_)
        layernorm10_ = self.layernorm10_(memory9_)
        fc10_ = self.fc10_(layernorm10_)
        dropout10_ = self.dropout10_(fc10_)
        fc11_ = self.fc11_(dropout10_)
        softmax11_ = F.softmax(fc11_, axis=-1)
        softmax_ = F.identity(softmax11_)
