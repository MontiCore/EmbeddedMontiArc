#ifndef CNNPREDICTOR_MEMORIESWITHPRODUCTKEYS_CONNECTOR_PREDICTOR
#define CNNPREDICTOR_MEMORIESWITHPRODUCTKEYS_CONNECTOR_PREDICTOR
...
#include <CNNLAOptimizer_memorieswithproductkeys_connector_predictor.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_memorieswithproductkeys_connector_predictor_0{
public:
    const std::string file_prefix = "model/memorieswithproductkeys.Network/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1", "data2"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 5}, {1, 5}, {1, 1}};
    ...
    explicit CNNPredictor_memorieswithproductkeys_connector_predictor_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_memorieswithproductkeys_connector_predictor_0(){
    ...
    void predict(const std::vector<float> &in_data_0_, const std::vector<float> &in_data_1_, const std::vector<float> &in_data_2_,
        ...
        input_temp.SyncCopyFromCPU(in_data_1_.data(), network_input_sizes[1]);
        ...
        input_temp.SyncCopyFromCPU(in_data_2_.data(), network_input_sizes[2]);
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_memorieswithproductkeys_connector_predictor optimizer_creator = CNNLAOptimizer_memorieswithproductkeys_connector_predictor();
...
#endif // CNNPREDICTOR_MEMORIESWITHPRODUCTKEYS_CONNECTOR_PREDICTOR
