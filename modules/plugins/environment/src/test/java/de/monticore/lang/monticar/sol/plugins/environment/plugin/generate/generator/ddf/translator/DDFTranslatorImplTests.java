...
public class DDFTranslatorImplTests {
    @Mock NotificationService notifications;
    @Mock EnvironmentGenerateConfiguration configuration;

    @InjectMocks DDFTranslatorImpl translator;

    @Test
    void testTranslate() {
        List<List<ASTInstruction>> partitions = new ArrayList<>();
        List<ASTInstruction> partition = new ArrayList<>();
        ASTInstallInstruction instruction = EnvironmentMill.installInstructionBuilder().build();
    ...
    void testPrintFilename() {
        ...
        assertEquals("\n#\n# Model.ddf\n#\n", translator.printer.getContent(), "Printed Filename does not match.");
