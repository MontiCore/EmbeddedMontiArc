...
class CNNSupervisedTrainer_vae_connector_decoder:
    ...
    def train(self, batch_size=64,
        ...
        if preprocessing:
            preproc_lib = "CNNPreprocessor_vae_connector_decoder_executor"
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    ...
                    data_ = [mx.nd.zeros((single_pu_batch_size, 1, 28, 28,), ctx=context) for context in mx_context]
                    ...
                    net_ret = [self._networks[0](encoding_[i]) for i in range(num_pus)]
                    data_ = [net_ret[i][0][0] for i in range(num_pus)]
                    [lossList[i].append(loss_function(data_[i], labels[0][i])) for i in range(num_pus)]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    ...
                    encoding_ = batch.data[0].as_in_context(mx_context[0])

                    data_ = mx.nd.zeros((single_pu_batch_size, 1, 28, 28,), ctx=mx_context[0])
                    ...
                    net_ret = self._networks[0](encoding_)
                    data_ = net_ret[0][0]
                    outputs.append(data_)
                    lossList.append(loss_function(data_, labels[0]))
