...
class CNNDataLoader_vae_connector_decoder:
    ...
    def load_vae_data(self, batch_size, shuffle=False, label_port="", input_names=[] ):
        self._input_names_ = input_names
        train_h5, test_h5 = self.load_h5_files(learning_method="vae")

        if label_port != "":
            if label_port not in self._input_names_:
                logging.error("The label_port '" + label_port + "' specifeied in the configuration, does not match the input ports.")
                sys.exit(1)
