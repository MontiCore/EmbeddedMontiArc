...
#ifndef CNNPREDICTOR_VAE_CONNECTOR_DECODER
#define CNNPREDICTOR_VAE_CONNECTOR_DECODER
...
#include <CNNLAOptimizer_vae_connector_decoder.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_vae_connector_decoder_0{
public:
    const std::string file_prefix = "model/vae.Decoder/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 8}};
    ...
    explicit CNNPredictor_vae_connector_decoder_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_vae_connector_decoder_0(){
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_vae_connector_decoder optimizer_creator = CNNLAOptimizer_vae_connector_decoder();
...
#endif // CNNPREDICTOR_VAE_CONNECTOR_DECODER
