...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.fc1_ = gluon.nn.Dense(units=200, use_bias=True, flatten=True)
            # fc1_, output shape: {[200,1,1]}

            self.relu1_ = gluon.nn.Activation(activation='relu')
            self.dropout1_ = gluon.nn.Dropout(rate=0.5)
            self.fc2_ = gluon.nn.Dense(units=400, use_bias=True, flatten=True)
            # fc2_, output shape: {[400,1,1]}

            self.relu2_ = gluon.nn.Activation(activation='relu')
            self.dropout2_ = gluon.nn.Dropout(rate=0.5)
    ...
    def hybrid_forward(self, F, encoding_):
        ...
        dropout1_ = self.dropout1_(relu1_)
        fc2_ = self.fc2_(dropout1_)
        relu2_ = self.relu2_(fc2_)
        dropout2_ = self.dropout2_(relu2_)
        fc3_ = self.fc3_(dropout2_)
        reshape3_ = F.reshape(fc3_, shape=(0,1,28,28))
        sigmoid3_ = self.sigmoid3_(reshape3_)
