        ...
        if len(self.loss_ctx_list) != 0:
            for i in range(len(self.loss_ctx_list)):
                ...
                if dict["loss"] == "kl_div_loss":
                    pdf = val_dict["pdf"]
                    regularizer_loss = self.kl_loss(F, pdf, loss_params, self.kl_weight)
                    ...
                    regularizer_loss = commitment_loss + codebook_loss

        if self.recon_loss == "bce":
            reconstruction_loss = - F.sum(data * F.log(pred + self.soft_zero) + (1 - data) * F.log(1 - pred + self.soft_zero), axis=0, exclude=True)
        elif self.recon_loss == "mse":
            reconstruction_loss = F.sum((pred - data)**2, axis=0, exclude=True)
...
class CNNAutoencoderTrainer:
    ...
    def train(self, batch_size=64,
        ...
        input_names = [ "encoding"]
        train_iter, test_iter, data_mean, data_std, _, _ = self._data_loader.load_vae_data(batch_size=batch_size,label_port=label_port, input_names=input_names)
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    indexed_labels = 0
                    if "encoding" == label_port:
                        label_ = gluon.utils.split_and_load(batch.label[indexed_labels], ctx_list=mx_context, even_split=False)
                        indexed_labels += 1
                        encoding_ = [mx.nd.one_hot(label_[i], 8) for i in range(num_pus)]
                    else:
                        encoding_ = gluon.utils.split_and_load(batch.data[0], ctx_list=mx_context, even_split=False)
                    ...
                    nd.waitall()
                    for i in range(num_pus):
                        feature_vec = encoder_nets[0]( encoding_[i])
