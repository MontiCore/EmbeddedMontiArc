...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.fc2_ = gluon.nn.Dense(units=200, use_bias=True, flatten=True)
            # fc2_, output shape: {[200,1,1]}

            self.relu2_ = gluon.nn.Activation(activation='relu')
            self.dropout2_ = gluon.nn.Dropout(rate=0.5)
            self.fc3_ = gluon.nn.Dense(units=16, use_bias=True, flatten=True)
            # fc3_, output shape: {[16,1,1]}


            self.reparameterize5_ = Reparameterize(shape=(8,), pdf='normal',)
