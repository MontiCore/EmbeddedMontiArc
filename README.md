...
**Attention:** Check the training context in the training configuration `src/model/lander/agent/network/LanderQNet.cnnt`. Only use the context "gpu" if you have installed the CUDA packages and the corresponding version of MXNet. Otherwise choose the context "cpu".
