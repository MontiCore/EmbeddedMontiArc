...
public class MassPointPhysicalVehicle extends PhysicalVehicle {
	...
	public MassPointPhysicalVehicle() {
		...
		// vehicle
		// no rotation
		this.position = new ArrayRealVector(new double[] { 0.0, 0.0, -this.getHeight() / 2 });
	...
	public void putOnSurface(double posX, double posY, double rotZ) {
		...
		double groundZ = WorldModel.getInstance().getGround(posX, posY, getGeometryPosition().getEntry(2))
				.doubleValue();
	...
	private void createMassPoints(double mass, double wheelDistLeftRightFrontSide, double wheelDistLeftRightBackSide,
		...
		RealVector localPositionFrontLeft = new ArrayRealVector(
				new double[] { -(wheelDistLeftRightFrontSide / 2), wheelDistToFront, 0.0 });
		RealVector localPositionFrontRight = new ArrayRealVector(
				new double[] { (wheelDistLeftRightFrontSide / 2), wheelDistToFront, 0.0 });
		RealVector localPositionBackLeft = new ArrayRealVector(
				new double[] { -(wheelDistLeftRightBackSide / 2), -wheelDistToBack, 0.0 });
		RealVector localPositionBackRight = new ArrayRealVector(
				new double[] { (wheelDistLeftRightBackSide / 2), -wheelDistToBack, 0.0 });
	...
	private void calcMassPointPosition() {
		for (MassPoint massPoint : massPoints) {
			...
			double groundZ = WorldModel.getInstance().getGround(massPointPosition.getEntry(0),
	...
	private RealVector calcGravityRelatedForces(MassPoint mp, double deltaT) {
		...
		if (groundContact > 0.0) {
			...
			RealVector backFront1 = (massPoints[MASS_POINT_TYPE_WHEEL_FRONT_LEFT.ordinal()].getPosition()
					.subtract(massPoints[MASS_POINT_TYPE_WHEEL_BACK_LEFT.ordinal()].getPosition()));
			RealVector backFront2 = (massPoints[MASS_POINT_TYPE_WHEEL_FRONT_RIGHT.ordinal()].getPosition()
					.subtract(massPoints[MASS_POINT_TYPE_WHEEL_BACK_RIGHT.ordinal()].getPosition()));
			RealVector leftRight1 = (massPoints[MASS_POINT_TYPE_WHEEL_FRONT_RIGHT.ordinal()].getPosition()
					.subtract(massPoints[MASS_POINT_TYPE_WHEEL_FRONT_LEFT.ordinal()].getPosition()));
			RealVector leftRight2 = (massPoints[MASS_POINT_TYPE_WHEEL_BACK_RIGHT.ordinal()].getPosition()
					.subtract(massPoints[MASS_POINT_TYPE_WHEEL_BACK_LEFT.ordinal()].getPosition()));
			...
			RealVector componentXYVector = new ArrayRealVector(
	...
	private RealVector calcCentripetalForce(MassPoint mp) {
		...
		if (curveRadiusSin != 0.0) {
			...
			RealVector curveRadiusVector = (massPoints[MASS_POINT_TYPE_WHEEL_FRONT_RIGHT.ordinal()].getPosition()
					.subtract(massPoints[MASS_POINT_TYPE_WHEEL_FRONT_LEFT.ordinal()].getPosition()));
