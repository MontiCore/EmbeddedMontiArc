...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.fc2_1_ = gluon.nn.Dense(units=256, use_bias=True, flatten=True)
            # fc2_1_, output shape: {[256,1,1]}

            self.relu2_1_ = gluon.nn.Activation(activation='relu')
            self.dropout2_1_ = gluon.nn.Dropout(rate=0.5)
            self.fc3_1_ = gluon.nn.Dense(units=256, use_bias=True, flatten=True)
            # fc3_1_, output shape: {[256,1,1]}
            ...
            self.fc2_2_ = gluon.nn.Dense(units=256, use_bias=True, flatten=True)
            # fc2_2_, output shape: {[256,1,1]}

            self.relu4_ = gluon.nn.Activation(activation='relu')
            self.dropout4_ = gluon.nn.Dropout(rate=0.5)
            self.fc4_ = gluon.nn.Dense(units=512, use_bias=True, flatten=True)
            # fc4_, output shape: {[512,1,1]}

            self.relu5_ = gluon.nn.Activation(activation='relu')
            self.dropout5_ = gluon.nn.Dropout(rate=0.5)
    ...
    def hybrid_forward(self, F, state_, action_):
        ...
        dropout2_1_ = self.dropout2_1_(relu2_1_)
        fc3_1_ = self.fc3_1_(dropout2_1_)
        action_ = self.input_normalization_action_(action_)
        fc2_2_ = self.fc2_2_(action_)
        add4_ = fc3_1_ + fc2_2_
        relu4_ = self.relu4_(add4_)
        dropout4_ = self.dropout4_(relu4_)
        fc4_ = self.fc4_(dropout4_)
        relu5_ = self.relu5_(fc4_)
        dropout5_ = self.dropout5_(relu5_)
        fc5_ = self.fc5_(dropout5_)
