...
#ifndef CNNPREDICTOR_MNIST_ENCODER
#define CNNPREDICTOR_MNIST_ENCODER
...
#include <CNNLAOptimizer_mnist_encoder.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_mnist_encoder_0{
public:
    const std::string file_prefix = "model/mnist.Encoder/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 1, 28, 28}, {1, 1}};
    ...
    explicit CNNPredictor_mnist_encoder_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_mnist_encoder_0(){
    ...
    void predict(const std::vector<float> &in_data_, const std::vector<float> &in_label_,
                 std::vector<float> &out_encoding_){
        ...
        assert((curr_output_size == out_encoding_.size()) || (curr_output_size == out_encoding_[0]));
        output[0].SyncCopyToCPU(&out_encoding_);
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_mnist_encoder optimizer_creator = CNNLAOptimizer_mnist_encoder();
...
#endif // CNNPREDICTOR_MNIST_ENCODER
