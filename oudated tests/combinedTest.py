...
data = yaml.safe_load(open("../architecture2.yaml"))
...
architecture = yaml.safe_load(open("../architecture2.yaml"))
for repoID in tqdm(data.keys(), desc="Migrating pipelines"):
  ...
  lfs_check = subprocess.run(["git", "lfs", "ls-files"], cwd="repos/" + data[repoID]["Name"], capture_output=True,
                             text=True)
  if lfs_check.stdout:
    pass  # remove_lfs("./repos/" + data[repoID]["Name"])
  for branch in branches:
    ...
    GitlabToGithub(repoID, architecture, config, "pipeline",
                   ["GITLABTOKEN", ("CI_API_V4_URL", "https://git.rwth-aachen.de/api/v4"),
                    ("CI_PROJECT_ID", repoID)])  # input("WAIT")

  repo.git.checkout("master")  # run_git_filter_repo("repos/" + data[repoID]["Name"])
