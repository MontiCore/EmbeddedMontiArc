 ...
 */
public class CompPowerInstSymbolCreator implements TagSymbolCreator {
  ...
  public void create(ASTTaggingUnit unit, TaggingResolver tagging) {
    if (unit.getQualifiedNameList().stream()
        .map(q -> q.toString())
        .filter(n -> n.endsWith("CompPower"))
        .count() == 0) {
      return; // the tagging model is not conform to the CompPower tagging schema
    }
    ...
    for (ASTTag element : unit.getTagBody().getTagList()) {
          ...
          .forEachOrdered(v ->
              element.getScopeList().stream()
                ...
                .map(Optional::get)
                .forEachOrdered(s -> tagging.addTag(s, new CompPowerInstSymbol(
                   NumericLiteral.getValue(v.getNumericLiteral()),
                   Unit.valueOf(v.getUnit()).asType(Energy.class)))));
    }
  }
  ...
  protected boolean checkUnit(ASTUnitTagValue unitTag) {
    ...
    if (!unit.isCompatible(Energy.UNIT)) {
      Log.error(String.format("0xT0002 The unit '%s' is not compatible to quantity kind 'Energy'",
          unitTag.getUnit()));
      return false;
    }
    return true;
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. CompPowerInst expects as scope kind 'NameScope'.",
