 ...
 */
public class ClassTagSymbolCreator implements TagSymbolCreator {
  ...
  public void create(ASTTaggingUnit unit, TaggingResolver tagging) {
    ...
    for (ASTTag element : unit.getTagBody().getTagList()) {
           element.getTagElementList().stream()
              .filter(t -> t.getName().equals("ClassTag"))
          ...
          .forEachOrdered(v ->
              element.getScopeList().stream()
                ...
                .map(Optional::get)
                .forEachOrdered(s -> tagging.addTag(s, new ClassTagSymbol(v))));
    }
  }
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. ClassTag expects as scope kind 'NameScope'.",
