...
def convert_params(hf_model, hf_tokenizer, hf_cfg, gluon_model, ctx):
...
def test_model(hf_model, hf_tokenizer, gluon_model, gpu):
    ...
    input_ids = [[47499, 39079, 6870, 1, 1], [24802, 37471, 6381, 1, 1], [21942, 26564, 26037, 1, 1]]
    valid_length = [3, 3, 3]

    gl_input_ids = mx.nd.array(input_ids)
    gl_valid_length = mx.nd.array(valid_length)

    gl_all_hiddens = gluon_model(
        gl_input_ids, 
    ...
    # outputting some slices to compare to 2.0.0 script
    print(len(gl_all_hiddens))
    print(gl_all_hiddens[0][0][0][300:350])
    print(gl_all_hiddens[0][1][2][300:350])
    print(gl_all_hiddens[0][2][4][300:350])
...
def convert_huggingface_model(args):
    ...
    gluon_model, _ = nlp.model.get_model('roberta_12_768_12', dataset_name='openwebtext_ccnews_stories_books_cased', use_decoder=False)
    convert_params(hf_model, hf_tokenizer, hf_model.config, gluon_model, ctx)
