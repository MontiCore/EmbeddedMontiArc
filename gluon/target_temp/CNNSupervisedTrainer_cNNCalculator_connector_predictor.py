...
class CNNSupervisedTrainer_cNNCalculator_connector_predictor:
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                with autograd.record():
                    ...
                    noise_ = batch.data[0].as_in_context(mx_context)

                    image_ = mx.nd.zeros((train_batch_size, 1, 64, 64,), ctx=mx_context)


                    lossList = []

                    image_ = self._networks[0](noise_)

                    lossList.append(loss_function(image_, labels[0]))
            ...
            for batch_i, batch in enumerate(train_test_iter):
                if True: 
                    ...
                    image_ = mx.nd.zeros((test_batch_size, 1, 64, 64,), ctx=mx_context)
