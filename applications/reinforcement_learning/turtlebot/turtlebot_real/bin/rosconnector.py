...
class RosConnector(object):
        ...
        if not rospy.is_shutdown():
            ...
            if self.is_terminated or self.in_reset or self.is_crash:
                ...
                if not self.__turtleBot_in_position:
                    ...
                    status_info = doTurtleBotAction(
                        self.__set_navigation_publisher, cmd_vel_linear, cmd_vel_angular
                    )

                    msgScan = None
                    while msgScan is None:
                        try:
                            msgScan = rospy.wait_for_message(
                                "/scan", LaserScan, timeout=5
                            )
                        except:
                            pass
                    ...
                    while odomMsg is None:
                        try:
                            odomMsg = rospy.wait_for_message(
                                "/odom", Odometry, timeout=5
                            )
                        except:
                            pass
    ...
    def positionResetter(self):
        ...
        (x_init, y_init, theta_init) = setTurtleBotPos(
            self.__set_position_publisher, self.X_INIT, self.Y_INIT, self.THETA_INIT
        )
        odomMsg = None
        while odomMsg is None:
            try:
