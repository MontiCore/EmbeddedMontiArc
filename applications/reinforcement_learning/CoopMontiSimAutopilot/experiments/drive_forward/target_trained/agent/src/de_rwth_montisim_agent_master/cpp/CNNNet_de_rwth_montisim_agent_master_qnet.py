...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.fc2_ = gluon.nn.Dense(units=512, use_bias=True, flatten=True)
            # fc2_, output shape: {[512,1,1]}

            self.relu2_ = gluon.nn.Activation(activation='relu')
            self.dropout2_ = gluon.nn.Dropout(rate=0.5)
            self.fc3_ = gluon.nn.Dense(units=256, use_bias=True, flatten=True)
            # fc3_, output shape: {[256,1,1]}

            self.relu3_ = gluon.nn.Activation(activation='relu')
            self.dropout3_ = gluon.nn.Dropout(rate=0.5)
            self.fc4_ = gluon.nn.Dense(units=3, use_bias=True, flatten=True)
            # fc4_, output shape: {[3,1,1]}

            self.tanh4_ = gluon.nn.Activation(activation='tanh')
    ...
    def hybrid_forward(self, F, state_):
        ...
        fc4_ = self.fc4_(dropout3_)
        tanh4_ = self.tanh4_(fc4_)
        action_ = F.identity(tanh4_)
