...
public class TicTacToeMiniMaxAgent extends LocalAgent {
    ...
    public String getMove(List<List<String>> state, List<String> legalMoves, String role, int numberOfEpisodesPlayed) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                String fieldStatus = state.get(i * 3 + j).get(3);
                board[i][j] = (fieldStatus.equals("b") ? "_" : fieldStatus).charAt(0); 
            }
        }
    
        TicTacToeAI.Move bestMove = TicTacToeAI.findBestMove(board);
        return role + " " + actionMoveMap.inverseBidiMap().getKey(bestMove.row * 3 + bestMove.col);
    }
