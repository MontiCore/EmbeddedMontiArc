...
import numpy as np
# from os import path
from gym_torcs import snakeoil3_gym_p3 as snakeoil3
#import snakeoil3_gym_python3 as snakeoil3
import numpy as np
import copy
...
class TorcsEnv:
    ...
    def step(self, u):
       #print("Step")
        ...
        obs = client.S.d
        #print(obs["img"])#debugging also 0 => client side error
        ...
        if (abs(track.any()) > 1.0 or abs(trackPos) > 1.0):
            reward = -200
        ...
        if self.time_step >= self.max_time_steps:  # Reset after x steps
            episode_terminate = True
            client.R.d['meta'] = True
    ...
    def obs_vision_to_image_rgb(self, obs_image_vec):
        ...
        sz = (64, 64)
        r = np.flipud(np.array(r).reshape(sz))
        g = np.flipud(np.array(g).reshape(sz))
        b = np.flipud(np.array(b).reshape(sz))
    ...
    def make_observaton(self, raw_obs):
        if self.vision is False:
            ...
            return Observation(focus=np.array(raw_obs['focus'], dtype=np.float32)/200.,
                               speedX=np.array(
                                   raw_obs['speedX'], dtype=np.float32)/300.0,
                               speedY=np.array(
                                   raw_obs['speedY'], dtype=np.float32)/300.0,
                               speedZ=np.array(
                                   raw_obs['speedZ'], dtype=np.float32)/300.0,
                               angle=np.array(
                                   raw_obs['angle'], dtype=np.float32)/3.1416,
                               damage=np.array(
                                   raw_obs['damage'], dtype=np.float32),
                               opponents=np.array(
                                   raw_obs['opponents'], dtype=np.float32)/200.,
                               rpm=np.array(
                                   raw_obs['rpm'], dtype=np.float32)/10000,
                               track=np.array(
                                   raw_obs['track'], dtype=np.float32)/200.,
                               trackPos=np.array(
                                   raw_obs['trackPos'], dtype=np.float32)/1.,
            ...
            image_rgb = self.obs_vision_to_image_rgb(
                raw_obs[names[-1]])  
