...
from gym_torcs.gym_torcs_p3 import TorcsEnv

from std_msgs.msg import Float32MultiArray, Bool, Int32, Float32, UInt8MultiArray
...
class RosTorcsConnector(object):
    ...
    vison_topic = '/torcs/vision'
    ros_update_rate = 1 #I think this is redundant, since snakeoil seems to be limited to 5 hz
    torcs_relaunch = 3
    ...
    def __init__(self,
        ...
        self._vision_publisher = rospy.Publisher(
            RosTorcsConnector.vison_topic, UInt8MultiArray, queue_size=1)

        # Initialize Subscribers
        self._action_subscriber = rospy.Subscriber(
            RosTorcsConnector.step_topic, Float32MultiArray, self.step, queue_size=1)

        self._reset_subscriber = rospy.Subscriber(
            RosTorcsConnector.reset_topic, Bool, self.reset, queue_size=1)
    ...
    def reset(self, msg=Bool(data=True)):
        ...
        if msg.data is True:
            ...
            img = obs.img.astype('uint8').tobytes()
            self._vision_publisher.publish(UInt8MultiArray(data=img))            
            self._runs += 1
    ...
    def step(self, msg):
        ...
        self.log(
            'Step {},; Action: {} ; Reward: {}, Resulting state: {}'
        ...
        img = obs.img
        img = img.astype('uint8').tobytes()
        self._vision_publisher.publish(UInt8MultiArray(data=img))
        self._lock.release()

    def adjust_action(self, action):
        adj_action = np.zeros(shape=(3,), dtype='float32')
        adj_action[0] = action[0] #* 0.25 #* 0.75 # * 0.5 # steering limiting factor
        #if self._speed < 1:
        adj_action[1] = (0.5 * action[1] + 0.5) 
        adj_action[2] = (0.5 * action[2] + 0.5) #squeeze to 0,1 range
        return adj_action
