...
def group_2x2(images):
    panels4x4 = []
    for i in range(0, len(images), 4):
        # vstack1, vstack2 = np.vstack([image[0] for image in images[i:i+2]]), np.vstack(images[i+2:i+4])
        vstack1, vstack2 = np.concatenate((images[i], images[i+1]), axis=1), np.concatenate((images[i+2], images[i+3]), axis=1)
        hstack = np.concatenate((vstack1, vstack2), axis=2)
        panels4x4.append(hstack)
    return np.stack(panels4x4)

def labels_as_segmented(data, label, thresh=5):
    res = []
    for d, l in zip(data, label):
        ...
        copy[d <= thresh] = 0
        res.append(copy)
    res = np.concatenate(res).reshape((-1,1,28,28))
...
def _main(args):
    ...
    fname_train = os.path.join(fashionmnist_path, 'fashionmnist_train_4x4.hdf5')
    fname_test = os.path.join(fashionmnist_path, 'fashionmnist_test_4x4.hdf5')
    ...
    train_data = group_2x2( (fashionmnist['train_data'] * 255).astype(np.uint8) )
    test_data = group_2x2( (fashionmnist['test_data'] * 255).astype(np.uint8) )
    ...
    train_label = group_2x2( labels_as_segmented(fashionmnist['train_data'], fashionmnist['train_label']) )
    test_label = group_2x2( labels_as_segmented(fashionmnist['test_data'], fashionmnist['test_label']) )
