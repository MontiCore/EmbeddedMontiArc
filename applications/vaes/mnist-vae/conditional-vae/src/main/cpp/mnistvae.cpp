...
int main(int argc, char* argv[]) {
    ...
    vector<float> label(1);
	label[0] = std::stoi(argv[2]);

    // EMADL generated VAE model -------------------------------------------------
    cvae_connector connector;
    connector.init();

	connector.data = conv_to< arma::Col<double> >::from( CNNTranslator::translateToCol(data,
				vector<size_t> {2}) );

    connector.label = conv_to< arma::ivec >::from( CNNTranslator::translateToIntCol(label,
				vector<size_t> {1}) );

    connector.execute();

	arma::cube img_result_cube = ((1-connector.res*3) + 1) * 127.5;
	// ----------------------------------------------------------------------------
	...
	cv::resize(result_img_cv, result_img_cv, cv::Size(), 10, 10, cv::INTER_LINEAR);
