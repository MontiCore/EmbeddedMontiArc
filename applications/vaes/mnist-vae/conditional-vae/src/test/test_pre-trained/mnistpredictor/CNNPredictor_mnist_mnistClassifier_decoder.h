...
#ifndef CNNPREDICTOR_MNIST_MNISTCLASSIFIER_DECODER
#define CNNPREDICTOR_MNIST_MNISTCLASSIFIER_DECODER
...
#include <CNNLAOptimizer_mnist_mnistClassifier_decoder.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_mnist_mnistClassifier_decoder_0{
public:
    const std::string file_prefix = "model/mnist.Decoder/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 2}, {1, 1}};
    ...
    explicit CNNPredictor_mnist_mnistClassifier_decoder_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_mnist_mnistClassifier_decoder_0(){
    ...
    void predict(const std::vector<float> &in_encoding_, const std::vector<float> &in_label_,
                 std::vector<float> &out_data_){


        NDArray input_temp;
        input_temp = NDArray(network_input_shapes[0], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_encoding_.data(), network_input_sizes[0]);
        ...
        input_temp.SyncCopyFromCPU(in_label_.data(), network_input_sizes[1]);
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_mnist_mnistClassifier_decoder optimizer_creator = CNNLAOptimizer_mnist_mnistClassifier_decoder();
...
#endif // CNNPREDICTOR_MNIST_MNISTCLASSIFIER_DECODER
