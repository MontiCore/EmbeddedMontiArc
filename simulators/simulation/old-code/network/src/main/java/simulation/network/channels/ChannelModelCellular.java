    ...
    public synchronized void computeTransmission(NetworkNode sender, NetworkNode otherNode, NetworkMessage message) {
        ...
        for (PhysicalObject physicalObject : NetworkSimulator.getInstance().getPhysicalObjects()) {
            if (physicalObject.getId() != sender.getPhysicalObject().getId() && physicalObject.getId() != otherNode.getPhysicalObject().getId()) {
                Vec3 senderPos = sender.getPhysicalObject().getGeometryPosition();
                Vec3 otherPos = physicalObject.getGeometryPosition();
                Vec3 diffVector = otherPos.subtract(senderPos);
                ...
                Map.Entry<Integer, Vec3> otherPositionInfo = Vehicle.getNearestPositionOnPositionList(positionList, otherPos, MULTI_PATH_ACCURACY_NEAREST_POSITION);
                Vec3 otherNearestPosOnRay = otherPositionInfo.getValue();
        ...
        Vec3 senderVelocity = sender.getPhysicalObject().getVelocity();
        double senderVelocityNorm = senderVelocity.getNorm();
        Vec3 otherVelocity = otherNode.getPhysicalObject().getVelocity();
        double otherVelocityNorm = otherVelocity.getNorm();
        Vec3 senderToOther = otherNode.getPhysicalObject().getGeometryPosition().subtract(sender.getPhysicalObject().getGeometryPosition());
        Vec3 otherToSender = senderToOther.mapMultiply(-1.0);
        ...
        if (node.getPhysicalObject().getPhysicalObjectType() == PhysicalObjectType.PHYSICAL_OBJECT_TYPE_NETWORK_CELL_BASE_STATION) {
            ...
            if (cellBaseStationAssignmentMap.containsKey(nodeID)) {
                ...
                if (cellBaseStationMap.containsKey(stationID)) {
                    Vec3 stationPos = cellBaseStationMap.get(stationID).getPhysicalObject().getGeometryPosition();
            ...
            synchronized (cellBaseStationMap) {
                for (Map.Entry<Long, NetworkNode> entry : cellBaseStationMap.entrySet()) {
                    Vec3 stationPos = entry.getValue().getPhysicalObject().getGeometryPosition();
