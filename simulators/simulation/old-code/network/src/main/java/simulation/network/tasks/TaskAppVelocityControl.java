...
import de.rwth.montisim.simulation.util.OrientedBoundingBox;
...
public class TaskAppVelocityControl extends NetworkTask {
    ...
    public static double computePriorityToTheRightVelocity(Map<String, Map.Entry<Long, List<Float>>> statusMap, Map<String, Map.Entry<Long, List<Float>>> trajectoryMap, List<Vertex> trajectory, double velocity, double compass, Vec3 gps, double length, double width, double height, String ipv6, Map<String, Long> priorityMap, AtomicLong priorityTakenTime) {
        ...
        for (Map.Entry<Long, List<Float>> statusEntry : statusMap.values()) {
            Vec3 otherGps = new Vec3(new double[]{statusEntry.getValue().get(0), statusEntry.getValue().get(1), statusEntry.getValue().get(2)});
        ...
        Vec3 frontGps = gps.copy();
        Vec3 addVector = new Vec3(new double[]{0.0, 1.0, 0.0});
        ...
        Map.Entry<Integer, Vec3> trajectoryInfo = Vehicle.getNearestPositionOnTrajectory(trajectory, frontGps, 20);
        int nextIndex = trajectoryInfo.getKey();
        double travelDistance = 0.0;
        Collection<EnvStreet> streetContainer = null;

        try {
            streetContainer = World.getInstance().getContainer().getStreets();
        } catch (Exception e) {
            e.printStackTrace();
            priorityMap.clear();
            return Double.MAX_VALUE;
        ...
        for (int i = nextIndex; i < Math.min(nextIndex + VELOCITY_CONTROL_PRIORITY_COMMON_MAX_TRAJECTORY_SIZE, trajectory.size()); ++i) {
            ...
            List<List<Vec3>> neighborPositionLists = Collections.synchronizedList(new LinkedList<>());
            Vec3 originalIntersectionPos = new Vec3(new double[]{0.0, 0.0, 0.0});
            ...
            for (EnvStreet street : streetContainer) {
                for (int j = 0; j < street.getNodes().size(); ++j) {
                    if (street.getNodes().get(j).getOsmId() == intersectionNodeOsmId) {
                        // Add nodes
                        Vec3 point = street.getNodes().get(j).getPoint();
                        originalIntersectionPos = new Vec3(new double[]{point.getX(), point.getY(), point.getZ()});

                        // Add as many nodes from previous neighbor list as possible
                        List<Vec3> prevList = Collections.synchronizedList(new LinkedList<>());
                        for (int k = j - VELOCITY_CONTROL_PRIORITY_STREET_NODE_AMOUNT; k < j; ++k) {
                            if (k >= 0 && k < street.getNodes().size()) {
                                Vec3 pointPrev = street.getNodes().get(k).getPoint();
                                Vec3 pos = new Vec3(new double[]{pointPrev.getX(), pointPrev.getY(), pointPrev.getZ()});
                                prevList.add(pos);
                            }
                        }
                        ...
                        List<Vec3> nextList = Collections.synchronizedList(new LinkedList<>());
                        for (int k = j + VELOCITY_CONTROL_PRIORITY_STREET_NODE_AMOUNT; k > j; --k) {
                            if (k >= 0 && k < street.getNodes().size()) {
                                Vec3 pointNext = street.getNodes().get(k).getPoint();
                                Vec3 pos = new Vec3(new double[]{pointNext.getX(), pointNext.getY(), pointNext.getZ()});
                                nextList.add(pos);
                            }
                        }
            ...
            if (neighborPositionListIndex >= 0 && neighborPositionListIndex < neighborPositionLists.size()) {
                ...
                List<Vec3> nearestList = neighborPositionLists.get(neighborPositionListIndex);
                if (nearestList.size() >= 2) {
                    Vec3 prevToIntersectionVector = nearestList.get(nearestList.size() - 1).subtract(nearestList.get(nearestList.size() - 2));

                    double minRightListIndexAngle = Math.PI;
                    for (int j = 0; j < neighborPositionLists.size(); ++j) {
                        List<Vec3> otherList = neighborPositionLists.get(j);
                        if (otherList.size() >= 2) {
                            Vec3 intersectionToOtherVector = otherList.get(otherList.size() - 2).subtract(otherList.get(otherList.size() - 1));
                            ...
                            Vec3 intersectionToOtherVectorRotated = rotationMatrixRight.operate(intersectionToOtherVector);
            ...
            if (rightListIndex != -1) {
                for (Map.Entry<String, Map.Entry<Long, List<Float>>> statusMapEntry : statusMap.entrySet()) {
                    if (trajectoryMap.containsKey(statusMapEntry.getKey())) {
                        ...
                        Vec3 otherFrontGps = new Vec3(new double[]{statusMapEntry.getValue().getValue().get(0), statusMapEntry.getValue().getValue().get(1), statusMapEntry.getValue().getValue().get(2)});
                        Vec3 otherAddVector = new Vec3(new double[]{0.0, 1.0, 0.0});
                        ...
                        if (!priorityMap.keySet().contains(statusMapEntry.getKey()) && otherPositionListIndex == rightListIndex) {
                            ...
                            List<Vec3> otherPositionList = Collections.synchronizedList(new LinkedList<>());
                            for (int j = 0; j < otherTrajectoryValues.size() / 3; ++j) {
                                Vec3 vectorPos = new Vec3(new double[]{otherTrajectoryValues.get(3 * j), otherTrajectoryValues.get((3 * j) + 1), otherTrajectoryValues.get((3 * j) + 2)});
                                otherPositionList.add(vectorPos);
                            }

                            Map.Entry<Integer, Vec3> otherTrajectoryInfo1 = Vehicle.getNearestPositionOnPositionList(otherPositionList, otherFrontGps, VELOCITY_CONTROL_PRIORITY_STREET_NODE_PRECISION);
                            Vec3 otherNearestPos1 = otherTrajectoryInfo1.getValue();
                            int otherNearestNextIndex1 = otherTrajectoryInfo1.getKey();

                            Map.Entry<Integer, Vec3> otherTrajectoryInfo2 = Vehicle.getNearestPositionOnPositionList(otherPositionList, originalIntersectionPos, VELOCITY_CONTROL_PRIORITY_STREET_NODE_PRECISION);
                            Vec3 otherNearestPos2 = otherTrajectoryInfo2.getValue();
                            ...
                            if (otherNearestPos2.getDistance(originalIntersectionPos) <= VELOCITY_CONTROL_PRIORITY_SECTOR_DISTANCE) {
                                List<Vec3> otherTravelPositions = Collections.synchronizedList(new LinkedList<>());
            ...
            for (String priorityEntry : priorityMap.keySet()) {
                if (priorityMap.get(priorityEntry).equals(intersectionNodeOsmId)) {
                    if (statusMap.containsKey(priorityEntry)) {
                        Vec3 otherGps = new Vec3(new double[]{statusMap.get(priorityEntry).getValue().get(0), statusMap.get(priorityEntry).getValue().get(1), statusMap.get(priorityEntry).getValue().get(2)});
                        ...
                        if (otherGps.getDistance(originalIntersectionPos) >= VELOCITY_CONTROL_PRIORITY_SECTOR_DISTANCE + otherLength) {
                            if (trajectoryMap.containsKey(priorityEntry)) {
                                ...
                                for (int j = 0; j < trajectoryMap.get(priorityEntry).getValue().size() / 3; ++j) {
                                    Vec3 vectorPos = new Vec3(new double[]{trajectoryMap.get(priorityEntry).getValue().get(3 * j), trajectoryMap.get(priorityEntry).getValue().get((3 * j) + 1), trajectoryMap.get(priorityEntry).getValue().get((3 * j) + 2)});
     ...
     * @param originalIntersectionPos Vec3 of original intersection node coordinates
     ...
     */
    public static int findNeighborNodeListForVehicle(List<List<Vec3>> neighborPositionLists, List<Float> trajectoryPositions, Vec3 originalIntersectionPos, Vec3 frontGps, double compass) {
        // Convert trajectory floats to real vectors
        List<Vec3> positionList = Collections.synchronizedList(new LinkedList<>());
        for (int i = 0; i < trajectoryPositions.size() / 3; ++i) {
            Vec3 vectorPos = new Vec3(new double[]{trajectoryPositions.get(3 * i), trajectoryPositions.get((3 * i) + 1), trajectoryPositions.get((3 * i) + 2)});
            positionList.add(vectorPos);
        }
        ...
        for (int i = 0; i < neighborPositionLists.size(); ++i) {
            Map.Entry<Integer, Vec3> neighborInfo = Vehicle.getNearestPositionOnPositionList(neighborPositionLists.get(i), frontGps, VELOCITY_CONTROL_PRIORITY_STREET_NODE_PRECISION);
            Vec3 nearestPos = neighborInfo.getValue();
        ...
        if (nearestListIndex >= 0 && nearestListIndex < neighborPositionLists.size()) {
            if (frontGps.getDistance(originalIntersectionPos) <= VELOCITY_CONTROL_PRIORITY_SECTOR_DISTANCE) {
                ...
                Vec3 compassVector = new Vec3(new double[]{0.0, 1.0, 0.0});
                Rotation rot = new Rotation(RotationOrder.XYZ, RotationConvention.VECTOR_OPERATOR, 0.0, 0.0, compass);
                RealMatrix rotationMatrix = new BlockRealMatrix(rot.getMatrix());
                compassVector = rotationMatrix.operate(compassVector);

                // Get vector from previous node to intersection node
                List<Vec3> nearestList = neighborPositionLists.get(nearestListIndex);
                if (nearestList.size() < 2) {
                    return -1;
                }
     ...
     */
    public static double computeCollisionAvoidanceVelocity(Map<String, Map.Entry<Long, List<Float>>> statusMap, Map<String, Map.Entry<Long, List<Float>>> trajectoryMap, List<Vertex> trajectory, double velocity, double compass, Vec3 gps, double length, double width, double height, String ipv6) {
        ...
        RealMatrix rotationMatrix = new BlockRealMatrix(rot.getMatrix());
        addVector = rotationMatrix.operate(addVector);
        Vec3 compassVector = addVector.copy();
        addVector = addVector.mapMultiply(0.5 * length);
        frontGps = frontGps.add(addVector);

        Map.Entry<Integer, Vec3> trajectoryInfo = Vehicle.getNearestPositionOnTrajectory(trajectory, frontGps, VELOCITY_CONTROL_AVOIDANCE_STREET_NODE_PRECISION);
        Vec3 nearestPos = trajectoryInfo.getValue();
        int nextIndex = trajectoryInfo.getKey();

        List<Vec3> shortTrajectory = Collections.synchronizedList(new LinkedList<>());
        ...
        for (int i = nextIndex; i < Math.min(nextIndex + VELOCITY_CONTROL_AVOIDANCE_COMMON_MAX_TRAJECTORY_SIZE, trajectory.size()); ++i) {
            Vec3 vector = new Vec3(new double[]{trajectory.get(i).getPosition().getEntry(0), trajectory.get(i).getPosition().getEntry(1), trajectory.get(i).getPosition().getEntry(2)});
        ...
        for (Map.Entry<String, Map.Entry<Long, List<Float>>> statusEntry : statusMap.entrySet()) {
            if (trajectoryMap.containsKey(statusEntry.getKey())) {
                ...
                Vec3 otherGps = new Vec3(new double[]{statusEntry.getValue().getValue().get(0), statusEntry.getValue().getValue().get(1), statusEntry.getValue().getValue().get(2)});
                ...
                Vec3 otherFrontGps = otherGps.copy();
                Vec3 otherAddVector = new Vec3(new double[]{0.0, 1.0, 0.0});
                Rotation otherRot = new Rotation(RotationOrder.XYZ, RotationConvention.VECTOR_OPERATOR, 0.0, 0.0, otherCompass);
                RealMatrix otherRotationMatrix = new BlockRealMatrix(otherRot.getMatrix());
                otherAddVector = otherRotationMatrix.operate(otherAddVector);
                Vec3 otherCompassVector = addVector.copy();
                otherAddVector = otherAddVector.mapMultiply(0.5 * otherLength);
                otherFrontGps = otherFrontGps.add(otherAddVector);

                List<Vec3> otherTrajectoryPositions = Collections.synchronizedList(new LinkedList<>());
                for (int i = 0; i < otherTrajectoryValues.size() / 3; ++i) {
                    Vec3 vectorPos = new Vec3(new double[]{otherTrajectoryValues.get(3 * i), otherTrajectoryValues.get((3 * i) + 1), otherTrajectoryValues.get((3 * i) + 2)});
                ...
                Map.Entry<Integer, Vec3> otherTrajectoryInfo = Vehicle.getNearestPositionOnPositionList(otherTrajectoryPositions, otherFrontGps, VELOCITY_CONTROL_AVOIDANCE_STREET_NODE_PRECISION);
                Vec3 otherNearestPos = otherTrajectoryInfo.getValue();
                int otherNextIndex = otherTrajectoryInfo.getKey();

                List<Vec3> otherShortTrajectory = Collections.synchronizedList(new LinkedList<>());
                ...
                for (int i = otherNextIndex; i < Math.min(otherNextIndex + VELOCITY_CONTROL_AVOIDANCE_COMMON_MAX_TRAJECTORY_SIZE, otherTrajectoryPositions.size()); ++i) {
                    Vec3 vector = new Vec3(new double[]{otherTrajectoryPositions.get(i).getEntry(0), otherTrajectoryPositions.get(i).getEntry(1), otherTrajectoryPositions.get(i).getEntry(2)});
                ...
                while (iterationCount < VELOCITY_CONTROL_AVOIDANCE_MAX_ITERATION_COUNT && travelDistance < checkDistance && otherTravelDistance < checkDistance) {
                    ...
                    if (travelDistance < minCollisionDistance) {
                        ...
                        Vec3 gpsOnTrajectory = gps.copy();
                        Vec3 compassOnTrajectory = compassVector.copy();
                        ...
                        Vec3 otherGpsOnTrajectory = otherGps.copy();
                        Vec3 otherCompassOnTrajectory = otherCompassVector.copy();
                        ...
                        if (collisionDetected) {
                            ...
                            if (velocity <= otherVelocity && collisionAngle < VELOCITY_CONTROL_AVOIDANCE_SIMILAR_DIRECTION_ANGLE) {
                                Vec3 gpsDiffOnTrajectory = otherGpsOnTrajectory.subtract(gpsOnTrajectory);
     ...
     */
    public static List<Map.Entry<Vec3, Vec3>> compute2DBoundaryVectors(Vec3 gps, Vec3 compass, double length, double width) {
        List<Map.Entry<Vec3, Vec3>> resultList = new LinkedList<>();

        // Compute compass vectors
        Vec3 compassVector = compass.copy();
        ...
        Vec3 backLeft2D = gps2D.add(compassVector2D.mapMultiply(-0.5 * length)).add(compassVectorOrthogonal2D.mapMultiply(-0.5 * width));
        Vec3 backRight2D = gps2D.add(compassVector2D.mapMultiply(-0.5 * length)).add(compassVectorOrthogonal2D.mapMultiply(0.5 * width));
        Vec3 frontLeft2D = gps2D.add(compassVector2D.mapMultiply(0.5 * length)).add(compassVectorOrthogonal2D.mapMultiply(-0.5 * width));
        Vec3 frontRight2D = gps2D.add(compassVector2D.mapMultiply(0.5 * length)).add(compassVectorOrthogonal2D.mapMultiply(0.5 * width));

        // Create map entries and insert them into list
        // Ordering is important here,
        Map.Entry<Vec3, Vec3> entryBack = new AbstractMap.SimpleEntry<Vec3, Vec3>(backLeft2D, backRight2D);
        Map.Entry<Vec3, Vec3> entryRight = new AbstractMap.SimpleEntry<Vec3, Vec3>(backRight2D, frontRight2D);
        Map.Entry<Vec3, Vec3> entryFront = new AbstractMap.SimpleEntry<Vec3, Vec3>(frontRight2D, frontLeft2D);
        Map.Entry<Vec3, Vec3> entryLeft = new AbstractMap.SimpleEntry<Vec3, Vec3>(frontLeft2D, backLeft2D);
     ...
     */
    public static boolean travelDistanceOnPositionList(List<Vec3> positionList, double travelDistance, Vec3 gps, Vec3 compass) {
        ...
        for (int i = 0; i < positionList.size(); ++i) {
            ...
            Vec3 currentToNext = positionList.get(i + 1).subtract(positionList.get(i));
            ...
            if (distanceOnTrajectory + currentToNextLength < travelDistance) {
                ...
                Vec3 compassResult = currentToNext.unitVector();
     ...
     */
    public static double computeLeftAngleSumPositionList(List<Vec3> positionList, Vec3 gps) {
        double leftTurnAngleSum = 0.0;
        Map.Entry<Integer, Vec3> trajectoryInfoLeftTurn = Vehicle.getNearestPositionOnPositionList(positionList, gps, VELOCITY_CONTROL_AVOIDANCE_STREET_NODE_PRECISION);
        ...
        for (int i = nextIndexLeftTurn - VELOCITY_CONTROL_AVOIDANCE_LEFT_TURN_NODE_CHECK_AMOUNT; i < 1 + nextIndexLeftTurn + VELOCITY_CONTROL_AVOIDANCE_LEFT_TURN_NODE_CHECK_AMOUNT; ++i) {
            if (i >= 0 && (i + 2) < positionList.size()) {
                Vec3 startToMiddle = positionList.get(i + 1).subtract(positionList.get(i));
                Vec3 middleToEnd = positionList.get(i + 2).subtract(positionList.get(i + 1));
                ...
                Vec3 middleToEndRotated = rotationMatrixLeftCurve.operate(middleToEnd);
