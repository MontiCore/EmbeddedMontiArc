...
#include "lifelongLanguageLearning_connector.h"
#include <armadillo>
...
int main(int argc, char* argv[]) {
    ...
    lifelongLanguageLearning_connector connector;
    connector.init();
    ...
    if (samplesFile.is_open() && sequencesFile.is_open() && lengthsFile.is_open()){
        ...
        while(getline(samplesFile, sample)){
            ...
            std::cout << "Prediction for sample " << std::to_string(sampleNum) << ": " << classIndex << std::endl;
            sampleNum++;
        }
