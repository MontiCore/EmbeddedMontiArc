...
import torcs_agent_network_reward_executor

class RewardFunction(object):
    def __init__(self):
        self.__reward_wrapper = torcs_agent_network_reward_executor.torcs_agent_network_reward_executor()
        self.__reward_wrapper.init()

    def reward(self, state, terminal):
        s = state.astype('double')
        t = bool(terminal)
        inp = torcs_agent_network_reward_executor.torcs_agent_network_reward_input()
        inp.state = s
        inp.isTerminal = t
...
class RosEnvironment(Environment):
    def __init__(self,
        ...
        self.__step_publisher = rospy.Publisher(action_topic, Float32MultiArray, queue_size=1)
    ...
    def step(self, action):
        action_rospy = Float32MultiArray()
    ...
    def __state_callback(self, data):
        self.__last_received_state = np.array(data.data, dtype='float32').reshape((29,))
