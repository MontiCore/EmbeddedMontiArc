...
from CNNNet_cvae_connector_decoder import Net_0

from CNNDatasets_cvae_connector_decoder import Dataset, TrainingDataset

log = logging.getLogger(__name__)

class CNNCreator_cvae_connector_decoder: # pylint: disable=invalid-name

    def __init__(self):
        self.weight_initializer = mx.init.Normal()
        self.networks = {}
        self._model_basedir_ = pathlib.Path("model", "cvae.Decoder")
    ...
    def construct(self, context, batch_size=1, data_mean=None, data_std=None):
        ...
        self.networks[0](mx.nd.zeros((batch_size, 2,), ctx=context[0]), mx.nd.zeros((batch_size, 1,), ctx=context[0]))
    ...
    def getInputs(self):
        ...
        inputs["encoding_"] = input_domains + (input_dimensions,)
        input_dimensions = (1,)
        input_domains = (int,0.0,9.0,)
        inputs["label_"] = input_domains + (input_dimensions,)
        return inputs
