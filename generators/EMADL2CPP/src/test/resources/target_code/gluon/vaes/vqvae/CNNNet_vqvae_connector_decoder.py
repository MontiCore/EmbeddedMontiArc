...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.upconvolution1_ = gluon.nn.Conv2DTranspose(channels=64,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution1_padding,
                use_bias=True)
            # upconvolution1_, output shape: {[64,14,14]}

            self.relu1_ = gluon.nn.Activation(activation='relu')
            self.upconvolution2_padding = (1,1)
            self.upconvolution2_ = gluon.nn.Conv2DTranspose(channels=32,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution2_padding,
                use_bias=True)
            # upconvolution2_, output shape: {[32,28,28]}
    ...
    def hybrid_forward(self, F, encoding_):
        ...
        upconvolution1_ = self.upconvolution1_(encoding_)
        relu1_ = self.relu1_(upconvolution1_)
        upconvolution2_ = self.upconvolution2_(relu1_)
        relu2_ = self.relu2_(upconvolution2_)
