...
class CNNCreator_cNNCalculator_connector_predictor1: # pylint: disable=invalid-name
    ...
    def validate_parameters(self):
        myelu1_temp = MyELU.MyELU()
        parameters_with_type = myelu1_temp.get_parameters()

        if 'alpha' in parameters_with_type:
             if isinstance(1,parameters_with_type['alpha']) == False:
                  raise TypeError('Wrong ' + str(type(1)) + ' of parameter \'alpha\' given in the model')
        else:
             raise AttributeError('Parameter of Layer not added to get_parameters function')


        myelu2_temp = MyELU.MyELU()
        parameters_with_type = myelu2_temp.get_parameters()
        ...
        myelu3_temp = MyELU.MyELU()
        parameters_with_type = myelu3_temp.get_parameters()
