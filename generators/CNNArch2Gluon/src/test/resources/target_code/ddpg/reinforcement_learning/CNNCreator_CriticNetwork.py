...
from CNNDatasets_CriticNetwork import Dataset, TrainingDataset

log = logging.getLogger(__name__)

class CNNCreator_CriticNetwork: # pylint: disable=invalid-name

    def __init__(self):
        self.weight_initializer = mx.init.Normal()
        self.networks = {}
        self._model_basedir_ = pathlib.Path("model", "CriticNetwork")
    ...
    def construct(self, context, batch_size=1, data_mean=None, data_std=None):
        ...
        self.networks[0](mx.nd.zeros((batch_size, 8,), ctx=context[0]), mx.nd.zeros((batch_size, 3,), ctx=context[0]))
