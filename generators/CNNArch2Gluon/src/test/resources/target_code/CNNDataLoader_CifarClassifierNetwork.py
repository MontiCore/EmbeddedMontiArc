...
from CNNDatasets_CifarClassifierNetwork import Dataset, TrainingDataset, RetrainingConf

class CNNDataLoader_CifarClassifierNetwork: # pylint: disable=invalid-name
    _input_names_ = ['data']
    _output_names_ = ['softmax_label']

    def __init__(self, data_cleaner):
        self._data_dir = pathlib.Path("data/CifarClassifierNetwork/")
        self._model_dir_   = os.path.join(os.path.join('./'), 'model', 'CifarClassifierNetwork')
    ...
    def load_retraining_conf(cls) -> RetrainingConf:
        ...
        try:
            ...
            path = "data/CifarClassifierNetwork/"
