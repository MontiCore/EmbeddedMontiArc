...
try:
    ...
    pass
from CNNDatasets_VQEncoder import TrainingDataset
...
class CNNSupervisedTrainer_VQEncoder:
    ...
    def train(self, batch_size=64,
        ...
        if preprocessing:
            preproc_lib = "CNNPreprocessor_VQEncoder_executor"
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    ...
                    encoding_ = [mx.nd.zeros((single_pu_batch_size, 1, 28, 28,), ctx=context) for context in mx_context]
                    ...
                    encoding_ = [net_ret[i][0][0] for i in range(num_pus)]
                    if (train_mask is not None or test_mask is not None) and epoch == 0:
                        train_mask = self.get_mask_array(encoding_[0].shape[0], train_mask)
                        test_mask = self.get_mask_array(encoding_[0].shape[0], test_mask)
                    if train_mask is not None:
                        outputs = [encoding_[0]]
                        [lossList[i].append(loss_function(mx.nd.squeeze(encoding_[i]), mx.nd.squeeze(labels[0][i]), mx.nd.expand_dims(train_mask, 1)).sum() / train_mask.sum().asscalar()) for i in range(num_pus)]
                    else:
                        [lossList[i].append(loss_function(encoding_[i], labels[0][i])) for i in range(num_pus)]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        encoding_ = mx.nd.zeros((single_pu_batch_size, 1, 28, 28,), ctx=mx_context[0])
                        ...
                        encoding_ = net_ret[0][0]
                        outputs.append(encoding_)
                        lossList.append(loss_function(encoding_, labels[0]))
