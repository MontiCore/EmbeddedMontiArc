...
try:
    ...
    pass
from CNNDataLoader_RNNsearch import TrainingDataset
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    ...
                    encoder_state_ = [mx.nd.zeros((single_pu_batch_size, 2, 1000,), ctx=context) for context in mx_context]
                    encoder_output_ = [mx.nd.zeros((single_pu_batch_size, 30, 2000,), ctx=context) for context in mx_context]
                    fc_output_ = [mx.nd.zeros((single_pu_batch_size, 30, 1000,), ctx=context) for context in mx_context]
                    decoder_state_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]
                    ...
                    net_ret = [self._networks[0](source_[i], encoder_state_[i]) for i in range(num_pus)]
                    fc_output_ = [net_ret[i][0][0] for i in range(num_pus)]
                    encoder_state_ = [net_ret[i][0][1] for i in range(num_pus)]
                    encoder_output_ = [net_ret[i][0][2] for i in range(num_pus)]
                    ...
                    net_ret = [self._networks[2](encoder_state_[i]) for i in range(num_pus)]
                    decoder_state_ = [net_ret[i][0][0] for i in range(num_pus)]

                    for j in range(num_pus):
                        for i in range(1, 30):
                            target_[i-1+1][j], decoder_state_[j], decoder_output_[j] = self._networks[3](decoder_state_[j], fc_output_[j], target_[i-1+0][j])[0]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        encoder_state_ = mx.nd.zeros((single_pu_batch_size, 2, 1000,), ctx=mx_context[0])
                        encoder_output_ = mx.nd.zeros((single_pu_batch_size, 30, 2000,), ctx=mx_context[0])
                        fc_output_ = mx.nd.zeros((single_pu_batch_size, 30, 1000,), ctx=mx_context[0])
                        decoder_state_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])
                        ...
                        net_ret = self._networks[0](source_, encoder_state_)
                        fc_output_ = net_ret[0][0]
                        encoder_state_ = net_ret[0][1]
                        encoder_output_ = net_ret[0][2]
                        ...
                        net_ret = self._networks[2](encoder_state_)
                        decoder_state_ = net_ret[0][0]
                        k = 3
                        ...
                        for i in range(1, 30):
                            ...
                            for seq, score, seqLossList, attention in sequences:
                                target_[i-1+0] = seq[-1]
                                net_ret = self._networks[3](decoder_state_, fc_output_, target_[i-1+0])

                                target_[i-1+1], decoder_state_, decoder_output_ = net_ret[0]
                                out = target_[i-1+1]
