...
from CNNDatasets_VGG16 import Dataset, TrainingDataset, RetrainingConf

class CNNDataLoader_VGG16: # pylint: disable=invalid-name
    _input_names_ = ['data']
    _output_names_ = ['predictions_label']

    def __init__(self, data_cleaner):
        self._data_dir = pathlib.Path("data/VGG16/")
        self._model_dir_   = os.path.join(os.path.join('./'), 'model', 'VGG16')
