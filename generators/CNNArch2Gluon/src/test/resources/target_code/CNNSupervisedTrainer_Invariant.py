...
try:
    ...
    pass
from CNNDataLoader_Invariant import TrainingDataset
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    labels = [gluon.utils.split_and_load(batch.label[i], ctx_list=mx_context, even_split=False) for i in range(3)]
                    ...
                    pred_ = [[mx.nd.zeros((single_pu_batch_size, 4,), ctx=context) for context in mx_context] for i in range(3)]
                    ...
                    net_ret = [self._networks[2](const1_[i]) for i in range(num_pus)]
                    pred_[2] = [net_ret[i][0][0] for i in range(num_pus)]
                    if (train_mask is not None or test_mask is not None) and epoch == 0:
                        train_mask = self.get_mask_array(pred_[2][0].shape[0], train_mask)
                        test_mask = self.get_mask_array(pred_[2][0].shape[0], test_mask)
                    if train_mask is not None:
                        outputs = [pred_[2][0]]
                        [lossList[i].append(loss_function(mx.nd.squeeze(pred_[2][i]), mx.nd.squeeze(labels[2][i]), mx.nd.expand_dims(train_mask, 1)).sum() / train_mask.sum().asscalar()) for i in range(num_pus)]
                    else:
                        [lossList[i].append(loss_function(pred_[2][i], labels[2][i])) for i in range(num_pus)]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        labels = [batch.label[i].as_in_context(mx_context[0]) for i in range(3)]
                        data_0_ = batch.data[0].as_in_context(mx_context[0])
                        data_1_ = batch.data[1].as_in_context(mx_context[0])

                        pred_ = [mx.nd.zeros((single_pu_batch_size, 4,), ctx=mx_context[0]) for i in range(3)]
                        ...
                        net_ret = self._networks[2](const1_)
                        pred_[2] = net_ret[0][0]
