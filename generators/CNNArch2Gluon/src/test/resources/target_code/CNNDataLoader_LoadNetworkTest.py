...
from CNNDatasets_LoadNetworkTest import Dataset, TrainingDataset, RetrainingConf

class CNNDataLoader_LoadNetworkTest: # pylint: disable=invalid-name
    _input_names_ = ['data']
    _output_names_ = ['softmax_label']

    def __init__(self, data_cleaner):
        self._data_dir = pathlib.Path("data/LoadNetworkTest/")
        self._model_dir_   = os.path.join(os.path.join('./'), 'model', 'LoadNetworkTest')
