...
from CNNNet_RNNencdec import Net_0
from CNNNet_RNNencdec import Net_1
from CNNNet_RNNencdec import Net_2
from CNNNet_RNNencdec import Net_3

from CNNDatasets_RNNencdec import Dataset, TrainingDataset

log = logging.getLogger(__name__)

class CNNCreator_RNNencdec: # pylint: disable=invalid-name

    def __init__(self):
        self.weight_initializer = mx.init.Normal()
        self.networks = {}
        self._model_basedir_ = pathlib.Path("model", "RNNencdec")
    ...
    def construct(self, context, batch_size=1, data_mean=None, data_std=None):
        ...
        self.networks[0](mx.nd.zeros((batch_size, 30,), ctx=context[0]), mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]), mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]))
        ...
        self.networks[2](mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]), mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]))
        ...
        self.networks[3](mx.nd.zeros((batch_size, 1,), ctx=context[0]), mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]), mx.nd.zeros((batch_size, 1,1000,), ctx=context[0]))
