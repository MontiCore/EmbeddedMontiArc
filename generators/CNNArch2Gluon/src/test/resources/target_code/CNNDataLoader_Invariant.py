...
from CNNDatasets_Invariant import Dataset, TrainingDataset, RetrainingConf

class CNNDataLoader_Invariant: # pylint: disable=invalid-name
    _input_names_ = ['data_0', 'data_1']
    _output_names_ = ['pred_0_label','pred_1_label','pred_2_label']

    def __init__(self, data_cleaner):
        self._data_dir = pathlib.Path("data/Invariant/")
        self._model_dir_   = os.path.join(os.path.join('./'), 'model', 'Invariant')
