...
#ifndef CNNPREDICTOR_INVARIANT
#define CNNPREDICTOR_INVARIANT
...
#include <CNNLAOptimizer_Invariant.h>
...
public:
    ...
    explicit CNNPredictor_Invariant_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_Invariant_0(){
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_Invariant optimizer_creator = CNNLAOptimizer_Invariant();
...
public:
    ...
    explicit CNNPredictor_Invariant_1(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_Invariant_1(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
...
public:
    ...
    explicit CNNPredictor_Invariant_2(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_Invariant_2(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
    ...
    void predict(const std::vector<float> &in_const1_,
        ...
        for (mx_uint i : curr_output_shape) curr_output_size *= i;
        //Fix due to a bug in the in how the output arrays are initialized when there are multiple outputs
        assert((curr_output_size == out_pred_2_.size()) || (curr_output_size == out_pred_2_[0]));
        output[0].SyncCopyToCPU(&out_pred_2_);
