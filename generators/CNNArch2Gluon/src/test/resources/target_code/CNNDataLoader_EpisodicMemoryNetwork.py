...
from CNNDatasets_EpisodicMemoryNetwork import Dataset, TrainingDataset, RetrainingConf

class CNNDataLoader_EpisodicMemoryNetwork: # pylint: disable=invalid-name
    _input_names_ = ['data']
    _output_names_ = ['softmax_label']

    def __init__(self, data_cleaner):
        self._data_dir = pathlib.Path("data/EpisodicMemoryNetwork/")
        self._model_dir_   = os.path.join(os.path.join('./'), 'model', 'EpisodicMemoryNetwork')
    ...
    def load_retraining_conf(cls) -> RetrainingConf:
        ...
        try:
            ...
            path = "data/EpisodicMemoryNetwork/"
