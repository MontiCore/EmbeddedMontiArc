        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    ...
                    indexed_data = 0
                    if "source" == "label":
                        source_ = gluon.utils.split_and_load(batch.label[indexed_labels], ctx_list=mx_context, even_split=False)
                        indexed_labels += 1
                    else:
                        source_ = gluon.utils.split_and_load(batch.data[indexed_data], ctx_list=mx_context, even_split=False)
                        indexed_data += 1
                    ...
                    nd.waitall()
                    for i in range(num_pus):
                        feature_vec = encoder_nets[0]( source_[i])
            ...
            for batch_i, batch in enumerate(test_iter):
                ...
                nd.waitall()
                for i in range(num_pus):
                    feature_vec = encoder_nets[0]( source_[i][:num_test_images])
