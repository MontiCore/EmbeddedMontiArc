  ...
  class FG_eval_CallIpopt121 {
    public:
      ...
      void operator()(ADvector &fg,const ADvector &vars) {
        ...
        fg[0] = x(1-1)*x(4-1)*(x(1-1)+x(2-1)+x(3-1))+x(3-1);

        //Updates
        {
        // Constraint Functions
            fg[ 1 +_V_CONSTR0 ]  = x(1-1)*x(2-1)*x(3-1)*x(4-1)-25;
            fg[ 1 +_V_CONSTR1 ]  = x(1-1)*x(1-1)+x(2-1)*x(2-1)+x(3-1)*x(3-1)+x(4-1)*x(4-1)-40;
            fg[ 1 +_V_CONSTR2 ]  = 40-x(1-1)*x(1-1)+x(2-1)*x(2-1)+x(3-1)*x(3-1)+x(4-1)*x(4-1);
        }
        return;
      }
...
class CallIpopt121
{
  ...
  public:
    static bool solveOptimizationProblemIpOpt(mat *x, double *y, colvec xOut, double yOut)
    {
      bool ok = true;
      ...
      // Initialize variable bounds & initial value
        ...
        for(int m = 0; m < 4E0;m++){
          for(int n = 0; n < 1E0;n++){
             int i = m*4E0 + n;
             vars_lowerbounds[ _V_x + i] = 5.0;
             vars_upperbounds[ _V_x + i] = 1.0;
          }
        }
