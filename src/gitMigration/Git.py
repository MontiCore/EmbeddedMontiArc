...
class Git:
    ...
    def remove_remote_origin(self, repo):
        ...
        try:
            ...
            logger.error(f"Fehler beim Zurücksetzen des Remote 'origin': {e}")
            typer.echo(
                "[red]Fehler beim Zurücksetzen des Remote 'origin'. PLease be carefull when manually pushing![/red]")
    ...
    def add_repos_as_subtree(self, target_repo_name, target_repo_namespace, architecture):
        ...
        if branch_name in target_repo.branches:
            user_input = input((f"[red]Branch {branch_name} already exists. Do you want to delete it? (y/n)"))
            ...
            for repoID in architecture.repoIDs:
                ...
                for branch in branches_to_be_migrated:
                    if multiple_branches:
                        summary[repoID][branch] = self.add_subtree_branch(target_repo, repo.name, repo.path,
                                                                          prefix=repo_namespace, branch=branch)
                    else:
                        summary[repoID][branch] = self.add_subtree(target_repo, repo.name, repo.path,
                                                                   prefix=repo_namespace, branch=branch)
