 ...
 */
public class IsPhysicalSymbolCreator implements TagSymbolCreator {
    ...
    final String rootCmp = // if-else does not work b/c of final (required by streams)
     ...
     for (ASTTag element : unit.getTagBody().getTagList()) {
            element.getTagElementList().stream()
              .filter(t -> t.getName().equals("IsPhysical"))
              ...
              .forEachOrdered(t ->
                  element.getScopeList().stream()
                    ...
                    .map(Optional::get)
                    .forEachOrdered(s -> tagging.addTag(s, new IsPhysicalSymbol())));
      }
    }
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. IsPhysical expects as scope kind 'NameScope'.",
