...
#include <msgs/tests_structs_NestedStruct.h>
#include <ros/ros.h>
class RosAdapter_tests_msg_nestedStructComp: public IAdapter{
	...
	void init(tests_msg_nestedStructComp* comp){
		...
		topic4Publisher = node_handle.advertise<msgs::tests_structs_NestedStruct>("topic4",5);
		ros::spin();
	}
	
	void publishtopic4Publisher(){
		msgs::tests_structs_NestedStruct tmpMsg;
