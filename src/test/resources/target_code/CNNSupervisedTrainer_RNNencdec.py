        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                with autograd.record():
                    ...
                    encoder_state_0_ = mx.nd.zeros((batch_size, 1, 1000,), ctx=mx_context)
                    encoder_state_1_ = mx.nd.zeros((batch_size, 1, 1000,), ctx=mx_context)
                    encoder_output_ = mx.nd.zeros((batch_size, 30, 1000,), ctx=mx_context)
                    decoder_state_0_ = mx.nd.zeros((batch_size, 1, 1000,), ctx=mx_context)
                    decoder_state_1_ = mx.nd.zeros((batch_size, 1, 1000,), ctx=mx_context)
                    ...
                    encoder_output_, encoder_state_0_, encoder_state_1_ = self._networks[0](source_, encoder_state_0_, encoder_state_1_)

                    target_[0] = self._networks[1](const1_)

                    lossList.append(loss_function(target_[0], labels[0]))
                    decoder_state_0_, decoder_state_1_ = self._networks[2](encoder_state_0_, encoder_state_1_)

                    for i in range(1, 30):
                        target_[i-1+1], decoder_state_0_, decoder_state_1_, decoder_output_ = self._networks[3](target_[i-1+0], decoder_state_0_, decoder_state_1_)
