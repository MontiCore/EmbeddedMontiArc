...
try:
    ...
    pass
from CNNDataLoader_RNNencdec import TrainingDataset
...
class CNNSupervisedTrainer_RNNencdec:
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    labels = [gluon.utils.split_and_load(batch.label[i], ctx_list=mx_context, even_split=False) for i in range(30)]
                    source_ = gluon.utils.split_and_load(batch.data[0], ctx_list=mx_context, even_split=False)

                    target_ = [[mx.nd.zeros((single_pu_batch_size, 1,), ctx=context) for context in mx_context] for i in range(30)]

                    encoder_state_0_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]
                    encoder_state_1_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]
                    encoder_output_ = [mx.nd.zeros((single_pu_batch_size, 30, 1000,), ctx=context) for context in mx_context]
                    decoder_state_0_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]
                    decoder_state_1_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]
                    decoder_output_ = [mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=context) for context in mx_context]

                    const1_ = [mx.nd.full((single_pu_batch_size, 1,), 1, ctx=context) for context in mx_context]
                    ...
                    net_ret = [self._networks[0](source_[i], encoder_state_0_[i], encoder_state_1_[i]) for i in range(num_pus)]
                    encoder_output_ = [net_ret[i][0][0] for i in range(num_pus)]
                    encoder_state_0_ = [net_ret[i][0][1] for i in range(num_pus)]
                    encoder_state_1_ = [net_ret[i][0][2] for i in range(num_pus)]

                    net_ret = [self._networks[1](const1_[i]) for i in range(num_pus)]
                    target_[0] = [net_ret[i][0][0] for i in range(num_pus)]
                    if (train_mask is not None or test_mask is not None) and epoch == 0:
                        train_mask = self.get_mask_array(target_[0][0].shape[0], train_mask)
                        test_mask = self.get_mask_array(target_[0][0].shape[0], test_mask)
                    if train_mask is not None:
                        outputs = [target_[0][0]]
                        [lossList[i].append(loss_function(mx.nd.squeeze(target_[0][i]), mx.nd.squeeze(labels[0][i]), mx.nd.expand_dims(train_mask, 1)).sum() / train_mask.sum().asscalar()) for i in range(num_pus)]
                    else:
                        [lossList[i].append(loss_function(target_[0][i], labels[0][i])) for i in range(num_pus)]


                    net_ret = [self._networks[2](encoder_state_0_[i], encoder_state_1_[i]) for i in range(num_pus)]
                    decoder_state_0_ = [net_ret[i][0][0] for i in range(num_pus)]
                    decoder_state_1_ = [net_ret[i][0][1] for i in range(num_pus)]

                    for j in range(num_pus):
                        for i in range(1, 30):
                            target_[i-1+1][j], decoder_state_0_[j], decoder_state_1_[j], decoder_output_[j] = self._networks[3](target_[i-1+0][j], decoder_state_0_[j], decoder_state_1_[j])[0]
                            lossList[j].append(loss_function(target_[i-1+1][j], labels[i-1+1][j]))
                            target_[i-1+1][j] = mx.nd.argmax(target_[i-1+1][j], axis=1).expand_dims(1) 
                            if use_teacher_forcing == "True":
                                target_[i-1+1][j] = mx.nd.expand_dims(labels[i-1+1][j], axis=1)
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        labels = [batch.label[i].as_in_context(mx_context[0]) for i in range(30)]
                        source_ = batch.data[0].as_in_context(mx_context[0])

                        target_ = [mx.nd.zeros((single_pu_batch_size, 1,), ctx=mx_context[0]) for i in range(30)]

                        encoder_state_0_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])
                        encoder_state_1_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])
                        encoder_output_ = mx.nd.zeros((single_pu_batch_size, 30, 1000,), ctx=mx_context[0])
                        decoder_state_0_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])
                        decoder_state_1_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])
                        decoder_output_ = mx.nd.zeros((single_pu_batch_size, 1, 1000,), ctx=mx_context[0])

                        const1_ = mx.nd.full((single_pu_batch_size, 1,), 1, ctx=mx_context[0])
                        ...
                        net_ret = self._networks[0](source_, encoder_state_0_, encoder_state_1_)
                        encoder_output_ = net_ret[0][0]
                        encoder_state_0_ = net_ret[0][1]
                        encoder_state_1_ = net_ret[0][2]
                        net_ret = self._networks[1](const1_)
                        target_[0] = net_ret[0][0]
                        outputs.append(target_[0])
                        lossList.append(loss_function(target_[0], labels[0]))
                        net_ret = self._networks[2](encoder_state_0_, encoder_state_1_)
                        decoder_state_0_ = net_ret[0][0]
                        decoder_state_1_ = net_ret[0][1]
                        k = 3
                        sequences = [([target_[1-1+0]], mx.nd.full((single_pu_batch_size, 1,), 1.0, ctx=mx_context[0]), [mx.nd.full((single_pu_batch_size, 64,), 0.0, ctx=mx_context[0])], [mx.nd.full((single_pu_batch_size, 64,), 0.0, ctx=mx_context[0])])]
                        ...
                        for i in range(1, 30):
                            ...
                            for seq, score, seqLossList, attention in sequences:
                                target_[i-1+0] = seq[-1]
                                net_ret = self._networks[3](target_[i-1+0], decoder_state_0_, decoder_state_1_)

                                target_[i-1+1], decoder_state_0_, decoder_state_1_, decoder_output_ = net_ret[0]
                                out = target_[i-1+1]
                                newLossList = seqLossList + [loss_function(target_[i-1+1], labels[i-1+1])]
