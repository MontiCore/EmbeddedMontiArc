...
try:
    ...
    pass
from CNNDataLoader_Show_attend_tell import TrainingDataset
...
class CNNSupervisedTrainer_Show_attend_tell:
    ...
    def train(self, batch_size=64,
        ...
        if preprocessing:
            preproc_lib = "CNNPreprocessor_Show_attend_tell_executor"
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    labels = [gluon.utils.split_and_load(batch.label[i], ctx_list=mx_context, even_split=False) for i in range(25)]
                    images_ = gluon.utils.split_and_load(batch.data[0], ctx_list=mx_context, even_split=False)

                    target_ = [[mx.nd.zeros((single_pu_batch_size, 1,), ctx=context) for context in mx_context] for i in range(25)]

                    features_output_ = [mx.nd.zeros((single_pu_batch_size, 256,), ctx=context) for context in mx_context]
                    decoder_state_0_ = [mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=context) for context in mx_context]
                    decoder_state_1_ = [mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=context) for context in mx_context]
                    decoder_output_ = [mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=context) for context in mx_context]

                    const1_ = [mx.nd.full((single_pu_batch_size, 1,), 0, ctx=context) for context in mx_context]
                    ...
                    net_ret = [self._networks[0](const1_[i]) for i in range(num_pus)]
                    ...
                    net_ret = [self._networks[1](images_[i]) for i in range(num_pus)]
                    features_output_ = [net_ret[i][0][0] for i in range(num_pus)]

                    for j in range(num_pus):
                        for i in range(1, 25):
                            target_[i-1+1][j], decoder_state_0_[j], decoder_state_1_[j], decoder_output_[j] = self._networks[2](features_output_[j], decoder_state_0_[j], target_[i-1+0][j], decoder_state_1_[j])[0]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        labels = [batch.label[i].as_in_context(mx_context[0]) for i in range(25)]
                        images_ = batch.data[0].as_in_context(mx_context[0])

                        target_ = [mx.nd.zeros((single_pu_batch_size, 1,), ctx=mx_context[0]) for i in range(25)]

                        features_output_ = mx.nd.zeros((single_pu_batch_size, 256,), ctx=mx_context[0])
                        decoder_state_0_ = mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=mx_context[0])
                        decoder_state_1_ = mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=mx_context[0])
                        decoder_output_ = mx.nd.zeros((single_pu_batch_size, 1, 512,), ctx=mx_context[0])

                        const1_ = mx.nd.full((single_pu_batch_size, 1,), 0, ctx=mx_context[0])
                        ...
                        for i in range(1, 25):
                            ...
                            for seq, score, seqLossList, attention in sequences:
                                target_[i-1+0] = seq[-1]
                                net_ret = self._networks[2](features_output_, decoder_state_0_, target_[i-1+0], decoder_state_1_)
