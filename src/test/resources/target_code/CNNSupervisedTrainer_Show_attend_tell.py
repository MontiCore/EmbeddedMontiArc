        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                with autograd.record():
                    ...
                    target_ = [mx.nd.zeros((batch_size, 1,), ctx=mx_context) for i in range(25)]

                    features_output_ = mx.nd.zeros((batch_size, 256,), ctx=mx_context)
                    decoder_state_0_ = mx.nd.zeros((batch_size, 1, 512,), ctx=mx_context)
                    decoder_state_1_ = mx.nd.zeros((batch_size, 1, 512,), ctx=mx_context)
                    decoder_output_ = mx.nd.zeros((batch_size, 1, 512,), ctx=mx_context)

                    const1_ = mx.nd.full((batch_size, 1,), 0, ctx=mx_context)

                    nd.waitall()

                    lossList = []

                    target_[0] = self._networks[0](const1_)

                    lossList.append(loss_function(target_[0], labels[0]))
                    features_output_ = self._networks[1](images_)

                    for i in range(1, 25):
                        target_[i-1+1], decoder_state_0_, decoder_state_1_, decoder_output_ = self._networks[2](features_output_, decoder_state_0_, target_[i-1+0], decoder_state_1_)
