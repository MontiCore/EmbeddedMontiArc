...
try:
    ...
    pass
from CNNDataLoader_torcs_agent_torcsAgent_actor import TrainingDataset
...
class CNNSupervisedTrainer_torcs_agent_torcsAgent_actor:
    ...
    def train(self, batch_size=64,
        ...
        if preprocessing:
            preproc_lib = "CNNPreprocessor_torcs_agent_torcsAgent_actor_executor"
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    ...
                    action_ = [mx.nd.zeros((single_pu_batch_size, 3,), ctx=context) for context in mx_context]
                    ...
                    net_ret = [self._networks[0](state_[i]) for i in range(num_pus)]
                    action_ = [net_ret[i][0][0] for i in range(num_pus)]
                    if (train_mask is not None or test_mask is not None) and epoch == 0:
                        train_mask = self.get_mask_array(action_[0].shape[0], train_mask)
                        test_mask = self.get_mask_array(action_[0].shape[0], test_mask)
                    if train_mask is not None:
                        outputs = [action_[0]]
                        [lossList[i].append(loss_function(mx.nd.squeeze(action_[i]), mx.nd.squeeze(labels[0][i]), mx.nd.expand_dims(train_mask, 1)).sum() / train_mask.sum().asscalar()) for i in range(num_pus)]
                    else:
                        [lossList[i].append(loss_function(action_[i], labels[0][i])) for i in range(num_pus)]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        state_ = batch.data[0].as_in_context(mx_context[0])

                        action_ = mx.nd.zeros((single_pu_batch_size, 3,), ctx=mx_context[0])
                        ...
                        net_ret = self._networks[0](state_)
                        action_ = net_ret[0][0]
                        outputs.append(action_)
                        lossList.append(loss_function(action_, labels[0]))
