...
class Net_0(gluon.Block):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            if data_mean:
                assert(data_std)
                self.input_normalization_graph_ = ZScoreNormalization(data_mean=data_mean['graph_'],
                                                                               data_std=data_std['graph_'])
            else:
                self.input_normalization_graph_ = NoNormalization()
            ...
            self.graphconv3_ = GraphConv(in_feats=1433,
                out_feats=16,
                bias=True)
            # graphconv3_, output shape: {[1,2708,16]}

            self.layerOne_output_ = gluon.nn.Activation(activation='relu')

            pass

    def forward(self, graph_, features_):
        graph_ = self.input_normalization_graph_(graph_)
        ...
        return [[layerOne_output_]]
#Stream 1
class Net_1(gluon.Block):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.graphconv5_ = GraphConv(in_feats=16,
                out_feats=7,
                bias=True)
    ...
    def forward(self, graph_, layerOne_output_):
        graph_ = self.input_normalization_graph_(graph_)
        graphconv5_ = self.graphconv5_(graph_, layerOne_output_)
        predictions_ = mx.nd.identity(graphconv5_)
