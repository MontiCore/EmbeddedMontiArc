...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            if data_mean:
                assert(data_std)
                self.input_normalization_encoding_ = ZScoreNormalization(data_mean=data_mean['encoding_'],
                                                                               data_std=data_std['encoding_'])
            else:
                self.input_normalization_encoding_ = NoNormalization()
            ...
            self.fc3_ = gluon.nn.Dense(units=3136, use_bias=True, flatten=True)
            # fc3_, output shape: {[3136,1,1]}

            self.relu3_ = gluon.nn.Activation(activation='relu')
            self.upconvolution3_padding = (1,1)
            self.upconvolution3_ = gluon.nn.Conv2DTranspose(channels=32,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution3_padding,
                use_bias=True)
            # upconvolution3_, output shape: {[32,14,14]}

            self.relu4_ = gluon.nn.Activation(activation='relu')
            self.upconvolution4_padding = (1,1)
            self.upconvolution4_ = gluon.nn.Conv2DTranspose(channels=64,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution4_padding,
                use_bias=True)
            # upconvolution4_, output shape: {[64,28,28]}

            self.relu5_ = gluon.nn.Activation(activation='relu')
            self.upconvolution5_padding = (1,1)
            self.upconvolution5_ = gluon.nn.Conv2DTranspose(channels=1,
                kernel_size=(3,3),
                strides=(1,1),
                padding=self.upconvolution5_padding,
                use_bias=True)
            # upconvolution5_, output shape: {[1,28,28]}

            self.sigmoid5_ = gluon.nn.Activation(activation='sigmoid')

            pass

    def hybrid_forward(self, F, encoding_, label_):
        encoding_ = self.input_normalization_encoding_(encoding_)
        ...
        concatenate3_ = F.concat(encoding_, flatten2_2_, dim=1)
        fc3_ = self.fc3_(concatenate3_)
        relu3_ = self.relu3_(fc3_)
        reshape3_ = F.reshape(relu3_, shape=(0,64,7,7))
        upconvolution3_ = self.upconvolution3_(reshape3_)
        relu4_ = self.relu4_(upconvolution3_)
        upconvolution4_ = self.upconvolution4_(relu4_)
        relu5_ = self.relu5_(upconvolution4_)
        upconvolution5_ = self.upconvolution5_(relu5_)
        sigmoid5_ = self.sigmoid5_(upconvolution5_)
        data_ = F.identity(sigmoid5_)
