...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.flatten2_1_ = gluon.nn.Flatten()
            # flatten2_1_, output shape: {[784,1,1]}

            if data_mean:
                assert(data_std)
                self.input_normalization_label_ = ZScoreNormalization(data_mean=data_mean['label_'],
                                                                               data_std=data_std['label_'])
            else:
                self.input_normalization_label_ = NoNormalization()


            self.flatten2_2_ = gluon.nn.Flatten()
            # flatten2_2_, output shape: {[10,1,1]}

            self.fc3_ = gluon.nn.Dense(units=784, use_bias=True, flatten=True)
            # fc3_, output shape: {[784,1,1]}

            self.relu3_ = gluon.nn.Activation(activation='relu')
            self.conv3_padding = Padding(padding=(0,0,0,0,1,0,1,0))
            self.conv3_ = gluon.nn.Conv2D(channels=32,
                kernel_size=(3,3),
                strides=(2,2),
                use_bias=True)
            # conv3_, output shape: {[32,14,14]}

            self.relu4_ = gluon.nn.Activation(activation='relu')
            self.conv4_padding = Padding(padding=(0,0,0,0,1,0,1,0))
            self.conv4_ = gluon.nn.Conv2D(channels=64,
                kernel_size=(3,3),
                strides=(2,2),
                use_bias=True)
            # conv4_, output shape: {[64,7,7]}

            self.relu5_ = gluon.nn.Activation(activation='relu')
            self.flatten5_ = gluon.nn.Flatten()
            # flatten5_, output shape: {[3136,1,1]}

            self.fc5_ = gluon.nn.Dense(units=16, use_bias=True, flatten=True)
            # fc5_, output shape: {[16,1,1]}

            self.relu6_ = gluon.nn.Activation(activation='relu')
            self.fc6_ = gluon.nn.Dense(units=4, use_bias=True, flatten=True)
            # fc6_, output shape: {[4,1,1]}


            self.reparameterize8_ = Reparameterize(shape=(2,), pdf='normal',)
    ...
    def hybrid_forward(self, F, data_, label_):
        loss_params = []
        data_ = self.input_normalization_data_(data_)
        flatten2_1_ = self.flatten2_1_(data_)
        label_ = self.input_normalization_label_(label_)
        onehot2_2_ = F.one_hot(indices=label_, depth=10)

        flatten2_2_ = self.flatten2_2_(onehot2_2_)
        concatenate3_ = F.concat(flatten2_1_, flatten2_2_, dim=1)
        fc3_ = self.fc3_(concatenate3_)
        relu3_ = self.relu3_(fc3_)
        reshape3_ = F.reshape(relu3_, shape=(0,1,28,28))
        conv3_padding = self.conv3_padding(reshape3_)
        conv3_ = self.conv3_(conv3_padding)
        relu4_ = self.relu4_(conv3_)
        conv4_padding = self.conv4_padding(relu4_)
        conv4_ = self.conv4_(conv4_padding)
        relu5_ = self.relu5_(conv4_)
        flatten5_ = self.flatten5_(relu5_)
        fc5_ = self.fc5_(flatten5_)
        relu6_ = self.relu6_(fc5_)
        fc6_ = self.fc6_(relu6_)

        split6_ = F.split(fc6_, axis=1, num_outputs=2)
        get7_1_ = split6_[0]
        get7_2_ = split6_[1]
        reparameterize8_ = self.reparameterize8_([get7_1_, get7_2_])
        loss_params.append(get7_1_)
        loss_params.append(get7_2_)

        encoding_ = F.identity(reparameterize8_)
