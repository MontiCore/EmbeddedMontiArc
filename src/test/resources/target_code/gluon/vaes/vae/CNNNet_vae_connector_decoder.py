...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.fc1_ = gluon.nn.Dense(units=3136, use_bias=True, flatten=True)
            # fc1_, output shape: {[3136,1,1]}

            self.relu1_ = gluon.nn.Activation(activation='relu')
            self.upconvolution1_padding = (1,1)
            self.upconvolution1_ = gluon.nn.Conv2DTranspose(channels=32,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution1_padding,
                use_bias=True)
            # upconvolution1_, output shape: {[32,14,14]}

            self.relu2_ = gluon.nn.Activation(activation='relu')
            self.upconvolution2_padding = (1,1)
            self.upconvolution2_ = gluon.nn.Conv2DTranspose(channels=64,
                kernel_size=(4,4),
                strides=(2,2),
                padding=self.upconvolution2_padding,
                use_bias=True)
            # upconvolution2_, output shape: {[64,28,28]}

            self.relu3_ = gluon.nn.Activation(activation='relu')
            self.upconvolution3_padding = (1,1)
            self.upconvolution3_ = gluon.nn.Conv2DTranspose(channels=1,
                kernel_size=(3,3),
                strides=(1,1),
                padding=self.upconvolution3_padding,
                use_bias=True)
            # upconvolution3_, output shape: {[1,28,28]}

            self.sigmoid3_ = gluon.nn.Activation(activation='sigmoid')

            pass


    def hybrid_forward(self, F, encoding_):
        encoding_ = self.input_normalization_encoding_(encoding_)
        fc1_ = self.fc1_(encoding_)
        relu1_ = self.relu1_(fc1_)
        reshape1_ = F.reshape(relu1_, shape=(0,64,7,7))
        upconvolution1_ = self.upconvolution1_(reshape1_)
        relu2_ = self.relu2_(upconvolution1_)
        upconvolution2_ = self.upconvolution2_(relu2_)
        relu3_ = self.relu3_(upconvolution2_)
        upconvolution3_ = self.upconvolution3_(relu3_)
        sigmoid3_ = self.sigmoid3_(upconvolution3_)
        data_ = F.identity(sigmoid3_)
