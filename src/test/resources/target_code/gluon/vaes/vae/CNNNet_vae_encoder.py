...
class Net_0(gluon.HybridBlock):
    def __init__(self, data_mean=None, data_std=None, mx_context=None, batch_size=None, **kwargs):
        ...
        with self.name_scope():
            ...
            self.conv1_padding = Padding(padding=(0,0,0,0,1,0,1,0))
            self.conv1_ = gluon.nn.Conv2D(channels=32,
                kernel_size=(3,3),
                strides=(2,2),
                use_bias=True)
            # conv1_, output shape: {[32,14,14]}

            self.relu1_ = gluon.nn.Activation(activation='relu')
            self.conv2_padding = Padding(padding=(0,0,0,0,1,0,1,0))
            self.conv2_ = gluon.nn.Conv2D(channels=64,
                kernel_size=(3,3),
                strides=(2,2),
                use_bias=True)
            # conv2_, output shape: {[64,7,7]}

            self.relu2_ = gluon.nn.Activation(activation='relu')
            self.flatten2_ = gluon.nn.Flatten()
            # flatten2_, output shape: {[3136,1,1]}

            self.fc2_ = gluon.nn.Dense(units=16, use_bias=True, flatten=True)
            # fc2_, output shape: {[16,1,1]}

            self.relu3_ = gluon.nn.Activation(activation='relu')
            self.fc3_ = gluon.nn.Dense(units=4, use_bias=True, flatten=True)
            # fc3_, output shape: {[4,1,1]}


            self.reparameterize5_ = Reparameterize(shape=(2,), pdf='normal',)
    ...
    def hybrid_forward(self, F, data_):
        ...
        flatten2_ = self.flatten2_(relu2_)
        fc2_ = self.fc2_(flatten2_)
        relu3_ = self.relu3_(fc2_)
        fc3_ = self.fc3_(relu3_)

        split3_ = F.split(fc3_, axis=1, num_outputs=2)
        get4_1_ = split3_[0]
        get4_2_ = split3_[1]
        reparameterize5_ = self.reparameterize5_([get4_1_, get4_2_])
        loss_params.append(get4_1_)
        loss_params.append(get4_2_)

        encoding_ = F.identity(reparameterize5_)
