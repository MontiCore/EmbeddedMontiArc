...
#ifndef CNNPREDICTOR_RNNENCDEC
#define CNNPREDICTOR_RNNENCDEC
...
#include <CNNLAOptimizer_RNNencdec.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_RNNencdec_0{
public:
    const std::string file_prefix = "model/RNNencdec/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1", "data2"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 30}, {1, 1, 1000}, {1, 1, 1000}};
    ...
    explicit CNNPredictor_RNNencdec_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_RNNencdec_0(){
    ...
    void predict(const std::vector<float> &in_source_, const std::vector<float> &in_encoder_state_0_, const std::vector<float> &in_encoder_state_1_,
                 std::vector<float> &out_encoder_output_, std::vector<float> &out_encoder_state_0_, std::vector<float> &out_encoder_state_1_){
        ...
        input_temp.SyncCopyFromCPU(in_encoder_state_0_.data(), network_input_sizes[1]);
        ...
        input_temp.SyncCopyFromCPU(in_encoder_state_1_.data(), network_input_sizes[2]);
        ...
        output[0].SyncCopyToCPU(&out_encoder_output_);
    
        curr_output_shape = output[1].GetShape();
        curr_output_size = 1;
        for (mx_uint i : curr_output_shape) curr_output_size *= i;
        //Fix due to a bug in the in how the output arrays are initialized when there are multiple outputs
        assert((curr_output_size == out_encoder_state_0_.size()) || (curr_output_size == out_encoder_state_0_[0]));
        output[1].SyncCopyToCPU(&out_encoder_state_0_);
    
        curr_output_shape = output[2].GetShape();
        curr_output_size = 1;
        for (mx_uint i : curr_output_shape) curr_output_size *= i;
        //Fix due to a bug in the in how the output arrays are initialized when there are multiple outputs
        assert((curr_output_size == out_encoder_state_1_.size()) || (curr_output_size == out_encoder_state_1_[0]));
        output[2].SyncCopyToCPU(&out_encoder_state_1_);
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_RNNencdec optimizer_creator = CNNLAOptimizer_RNNencdec();
...
class CNNPredictor_RNNencdec_1{
public:
    const std::string file_prefix = "model/RNNencdec/model_1_newest";
    ...
    explicit CNNPredictor_RNNencdec_1(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_RNNencdec_1(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
...
class CNNPredictor_RNNencdec_2{
public:
    const std::string file_prefix = "model/RNNencdec/model_2_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 1, 1000}, {1, 1, 1000}};
    std::vector<mx_uint> network_input_sizes;
    std::vector<std::vector<std::string>> network_arg_names;
    std::vector<Executor *> network_handles;
    ...
    explicit CNNPredictor_RNNencdec_2(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_RNNencdec_2(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
    ...
    void predict(const std::vector<float> &in_encoder_state_0_, const std::vector<float> &in_encoder_state_1_,
                 std::vector<float> &out_decoder_state_0_, std::vector<float> &out_decoder_state_1_){


        NDArray input_temp;
        input_temp = NDArray(network_input_shapes[0], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_encoder_state_0_.data(), network_input_sizes[0]);
        input_temp.CopyTo(&(network_handles[0]->arg_dict()[network_input_keys[0]]));
        input_temp = NDArray(network_input_shapes[1], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_encoder_state_1_.data(), network_input_sizes[1]);
        input_temp.CopyTo(&(network_handles[0]->arg_dict()[network_input_keys[1]]));
        NDArray::WaitAll();
        ...
        output[0].SyncCopyToCPU(&out_decoder_state_0_);
        ...
        output[1].SyncCopyToCPU(&out_decoder_state_1_);
...
class CNNPredictor_RNNencdec_3{
public:
    const std::string file_prefix = "model/RNNencdec/model_3_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data0", "data1", "data2"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 1}, {1, 1, 1000}, {1, 1, 1000}};
    std::vector<mx_uint> network_input_sizes;
    std::vector<std::vector<std::string>> network_arg_names;
    std::vector<Executor *> network_handles;
    ...
    explicit CNNPredictor_RNNencdec_3(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_RNNencdec_3(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
    ...
    void predict(const std::vector<float> &in_target_999999_, const std::vector<float> &in_decoder_state_0_, const std::vector<float> &in_decoder_state_1_,
        ...
        input_temp.CopyTo(&(network_handles[0]->arg_dict()[network_input_keys[1]]));
        input_temp = NDArray(network_input_shapes[2], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_decoder_state_1_.data(), network_input_sizes[2]);
        input_temp.CopyTo(&(network_handles[0]->arg_dict()[network_input_keys[2]]));
        NDArray::WaitAll();
