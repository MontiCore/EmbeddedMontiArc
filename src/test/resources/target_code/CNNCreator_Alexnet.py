...
from CNNDataLoader_Alexnet import CNNDataLoader_Alexnet as CNNDataLoader
...
#************* Start Stream 0*****************************
        ...
        conv1_ = tf.keras.layers.Conv2D(96, 
                                                 kernel_size=(11,11), 
                                                 strides=(4,4), 
                                                 use_bias=True,
                                                 padding="no_loss",
                                                 ...
                                                 name="conv1_")(data_)
        # conv1_, output shape: {[55,55,96]}

        lrn1_ = tf.keras.layers.Lambda(lambda x: tf.nn.local_response_normalization(input=x,
                                                                                              depth_radius=5 / 2, #instead element.depth_radius?
                                                                                              bias=2, #instead element.bias?												 
                                                                                              alpha=0.0001,
                                                                                              beta=0.75,
                                                                                              name="lrn1_"))(conv1_)
        ...
        pool1_ = tf.keras.layers.MaxPool2D(pool_size = (3,3), #or element.poolsize?
            ...
            name="pool1_")(lrn1_)
        # pool1_, output shape: {[27,27,96]}

        relu1_ = tf.keras.layers.Activation(activation = "relu", name="relu1_")(pool1_)

        split1_ = tf.keras.layers.Lambda(lambda x: tf.split(x,num_or_size_splits=2,axis=3))(relu1_)     
        # split1_, output shape: {[27,27,48]}

        get2_1_ = split1_[0]

        conv2_1_ = tf.keras.layers.Conv2D(128, 
                                                 kernel_size=(5,5), 
                                                 strides=(1,1), 
                                                 use_bias=True,
                                                 padding="same",
                                                 kernel_regularizer=self._regularizer_, 
                                                 kernel_constraint=self._weight_constraint_, 
                                                 name="conv2_1_")(get2_1_)
        # conv2_1_, output shape: {[27,27,128]}

        lrn2_1_ = tf.keras.layers.Lambda(lambda x: tf.nn.local_response_normalization(input=x,
                                                                                              depth_radius=5 / 2, #instead element.depth_radius?
                                                                                              bias=2, #instead element.bias?												 
                                                                                              alpha=0.0001,
                                                                                              beta=0.75,
                                                                                              name="lrn2_1_"))(conv2_1_)

        pool2_1_ = tf.keras.layers.MaxPool2D(pool_size = (3,3), #or element.poolsize?
            strides = (2,2), #or element.strides?  (plural)
            padding="no_loss",            
            data_format = "channels_last",
            name="pool2_1_")(lrn2_1_)
        # pool2_1_, output shape: {[13,13,128]}

        relu2_1_ = tf.keras.layers.Activation(activation = "relu", name="relu2_1_")(pool2_1_)

        get2_2_ = split1_[1]

        conv2_2_ = tf.keras.layers.Conv2D(128, 
                                                 kernel_size=(5,5), 
                                                 strides=(1,1), 
                                                 use_bias=True,
                                                 padding="same",
                                                 kernel_regularizer=self._regularizer_, 
                                                 kernel_constraint=self._weight_constraint_, 
                                                 name="conv2_2_")(get2_2_)
        # conv2_2_, output shape: {[27,27,128]}

        lrn2_2_ = tf.keras.layers.Lambda(lambda x: tf.nn.local_response_normalization(input=x,
                                                                                              depth_radius=5 / 2, #instead element.depth_radius?
                                                                                              bias=2, #instead element.bias?												 
                                                                                              alpha=0.0001,
                                                                                              beta=0.75,
                                                                                              name="lrn2_2_"))(conv2_2_)

        pool2_2_ = tf.keras.layers.MaxPool2D(pool_size = (3,3), #or element.poolsize?
            strides = (2,2), #or element.strides?  (plural)
            padding="no_loss",            
            data_format = "channels_last",
            name="pool2_2_")(lrn2_2_)
        # pool2_2_, output shape: {[13,13,128]}

        relu2_2_ = tf.keras.layers.Activation(activation = "relu", name="relu2_2_")(pool2_2_)

        concatenate3_ = tf.keras.layers.Concatenate(axis=-1, name="concatenate3_")([relu2_1_,  relu2_2_])
        # concatenate3_, output shape: {[13,13,256]}

        conv3_ = tf.keras.layers.Conv2D(384, 
                                                 kernel_size=(3,3), 
                                                 strides=(1,1), 
                                                 use_bias=True,
                                                 padding="same",
                                                 kernel_regularizer=self._regularizer_, 
                                                 kernel_constraint=self._weight_constraint_, 
                                                 name="conv3_")(concatenate3_)
        # conv3_, output shape: {[13,13,384]}

        relu3_ = tf.keras.layers.Activation(activation = "relu", name="relu3_")(conv3_)

        split3_ = tf.keras.layers.Lambda(lambda x: tf.split(x,num_or_size_splits=2,axis=3))(relu3_)     
        # split3_, output shape: {[13,13,192]}

        get4_1_ = split3_[0]

        conv4_1_ = tf.keras.layers.Conv2D(192, 
                                                 kernel_size=(3,3), 
                                                 strides=(1,1), 
                                                 use_bias=True,
                                                 padding="same",
        ...
        # conv5_1_, output shape: {[13,13,128]}

        pool5_1_ = tf.keras.layers.MaxPool2D(pool_size = (3,3), #or element.poolsize?
            strides = (2,2), #or element.strides?  (plural)
            padding="no_loss",            
            data_format = "channels_last",
            name="pool5_1_")(conv5_1_)
        # pool5_1_, output shape: {[6,6,128]}

        relu5_1_ = tf.keras.layers.Activation(activation = "relu", name="relu5_1_")(pool5_1_)

        get4_2_ = split3_[1]

        conv4_2_ = tf.keras.layers.Conv2D(192, 
                                                 kernel_size=(3,3), 
                                                 strides=(1,1), 
                                                 use_bias=True,
                                                 padding="same",
        ...
        # conv5_2_, output shape: {[13,13,128]}

        pool5_2_ = tf.keras.layers.MaxPool2D(pool_size = (3,3), #or element.poolsize?
            strides = (2,2), #or element.strides?  (plural)
            padding="no_loss",            
            data_format = "channels_last",
            name="pool5_2_")(conv5_2_)
        # pool5_2_, output shape: {[6,6,128]}

        relu5_2_ = tf.keras.layers.Activation(activation = "relu", name="relu5_2_")(pool5_2_)

        concatenate6_ = tf.keras.layers.Concatenate(axis=-1, name="concatenate6_")([relu5_1_,  relu5_2_])
        # concatenate6_, output shape: {[6,6,256]}

        fc6_ = tf.keras.layers.Flatten()(concatenate6_)

        fc6_ = tf.keras.layers.Dense(4096,
                                                use_bias=False,
                                                kernel_regularizer=self._regularizer_, 
                                                kernel_constraint=self._weight_constraint_, 
                                                name="fc6_")(fc6_)

        relu6_ = tf.keras.layers.Activation(activation = "relu", name="relu6_")(fc6_)

        dropout6_ = tf.keras.layers.Dropout(rate = 0.5, name="dropout6_")(relu6_)


        fc7_ = tf.keras.layers.Dense(4096,
                                                use_bias=False,
                                                kernel_regularizer=self._regularizer_, 
                                                kernel_constraint=self._weight_constraint_, 
                                                name="fc7_")(dropout6_)

        relu7_ = tf.keras.layers.Activation(activation = "relu", name="relu7_")(fc7_)

        dropout7_ = tf.keras.layers.Dropout(rate = 0.5, name="dropout7_")(relu7_)


        fc8_ = tf.keras.layers.Dense(10,
                                                use_bias=False,
                                                kernel_regularizer=self._regularizer_, 
                                                kernel_constraint=self._weight_constraint_, 
                                                name="fc8_")(dropout7_)
