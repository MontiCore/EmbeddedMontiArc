...
try:
    ...
    pass
from CNNDataLoader_RNNtest import TrainingDataset
        ...
        for epoch in range(begin_epoch, begin_epoch + num_epoch):
            ...
            for batch_i, batch in enumerate(train_iter):
                ...
                with autograd.record():
                    labels = [gluon.utils.split_and_load(batch.label[i], ctx_list=mx_context, even_split=False) for i in range(5)]
                    source_ = gluon.utils.split_and_load(batch.data[0], ctx_list=mx_context, even_split=False)

                    target_ = [[mx.nd.zeros((single_pu_batch_size, 30000,), ctx=context) for context in mx_context] for i in range(5)]
                    ...
                    net_ret = [self._networks[0](source_[i]) for i in range(num_pus)]
                    ...
                    for j in range(num_pus):
                        for i in range(1, 5):
                            target_[i-1+1][j] = self._networks[1](target_[i-1+0][j])[0]
            ...
            if eval_train:
                ...
                for batch_i, batch in enumerate(train_iter):
                    if True: # This statement is needed because of different indentations in FreeMarker
                        ...
                        labels = [batch.label[i].as_in_context(mx_context[0]) for i in range(5)]
                        source_ = batch.data[0].as_in_context(mx_context[0])

                        target_ = [mx.nd.zeros((single_pu_batch_size, 30000,), ctx=mx_context[0]) for i in range(5)]
                        ...
                        net_ret = self._networks[0](source_)
                        ...
                        for i in range(1, 5):
                            ...
                            for seq, score, seqLossList, attention in sequences:
                                target_[i-1+0] = seq[-1]
                                net_ret = self._networks[1](target_[i-1+0])

                                target_[i-1+1] = net_ret[0]
                                out = target_[i-1+1]
