...
#ifndef CNNPREDICTOR_MULTIPLESTREAMS
#define CNNPREDICTOR_MULTIPLESTREAMS
...
#include <CNNLAOptimizer_MultipleStreams.h>

using namespace mxnet::cpp;    
    
class CNNPredictor_MultipleStreams_0{
public:
    const std::string file_prefix = "model/MultipleStreams/model_0_newest";
    
    //network
    const std::vector<std::string> network_input_keys = {
        "data"
    };
    const std::vector<std::vector<mx_uint>> network_input_shapes = {{1, 10}};
    ...
    explicit CNNPredictor_MultipleStreams_0(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_MultipleStreams_0(){
    ...
    void predict(const std::vector<float> &in_data_0_,
                 std::vector<float> &out_pred_0_){


        NDArray input_temp;
        input_temp = NDArray(network_input_shapes[0], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_data_0_.data(), network_input_sizes[0]);
        ...
        assert((curr_output_size == out_pred_0_.size()) || (curr_output_size == out_pred_0_[0]));
        output[0].SyncCopyToCPU(&out_pred_0_);
    ...
    void init(const std::string &file_prefix,
        ...
        CNNLAOptimizer_MultipleStreams optimizer_creator = CNNLAOptimizer_MultipleStreams();
...
class CNNPredictor_MultipleStreams_1{
public:
    const std::string file_prefix = "model/MultipleStreams/model_1_newest";
    ...
    explicit CNNPredictor_MultipleStreams_1(){
        init(file_prefix, network_input_keys, network_input_shapes);
    }

    ~CNNPredictor_MultipleStreams_1(){
        for(Executor * handle : network_handles){
            delete handle;
        }
        MXNotifyShutdown();
    ...
    void predict(const std::vector<float> &in_data_1_,
                 std::vector<float> &out_pred_1_){


        NDArray input_temp;
        input_temp = NDArray(network_input_shapes[0], ctx, false, dtype);
        input_temp.SyncCopyFromCPU(in_data_1_.data(), network_input_sizes[0]);
        input_temp.CopyTo(&(network_handles[0]->arg_dict()[network_input_keys[0]]));
        NDArray::WaitAll();
        ...
        for (mx_uint i : curr_output_shape) curr_output_size *= i;
        //Fix due to a bug in the in how the output arrays are initialized when there are multiple outputs
        assert((curr_output_size == out_pred_1_.size()) || (curr_output_size == out_pred_1_[0]));
        output[0].SyncCopyToCPU(&out_pred_1_);
...
#endif // CNNPREDICTOR_MULTIPLESTREAMS
