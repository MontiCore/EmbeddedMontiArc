...
<#if tc.architecture.useDgl && input_name?index == 0>
                    ${input_name} = [graph]
<#else>
                    ${input_name} = gluon.utils.split_and_load(batch.data[data_index], ctx_list=mx_context, even_split=False)[0]
                    data_index += 1
</#if>
</#if>
</#list>
...
<#list tc.architectureOutputSymbols as output>
                    ${tc.getName(output)} = [mx.nd.zeros((${tc.join(output.ioDeclaration.type.dimensions, ", ")},), ctx=context) for context in mx_context]<#sep>,
</#list>
</#if>

<#list tc.getLayerVariableMembers()?keys as member>
                    ${member} = [mx.nd.zeros((${tc.join(tc.cutDimensions(tc.getLayerVariableMembers()[member]), ", ")},), ctx=context) for context in mx_context]
</#list>

<#list tc.architecture.constants as constant>
                    ${tc.getName(constant)} = [mx.nd.full((1,), ${constant.intValue?c}, ctx=context) for context in mx_context]
</#list>
...
<#list tc.getStreamOutputNames(networkInstruction.body, true) as outputName>
<#if tc.getNameWithoutIndex(outputName) == tc.outputName>
                    outputs.append(${tc.outputName}[0])
