...
def load_data(path, dataset):
    ...
    idx = np.array(content[:, 0], dtype=np.dtype(str))
    idx_map = {j: i for i, j in enumerate(idx)}

    edges_unordered = np.genfromtxt(path+dataset+".cites", dtype=np.dtype(str))
...
if __name__ == "__main__":
    ...
    adj_matrix, feat_matrix, labels = load_data("citeseer/", "citeseer")
    ...
    with h5py.File("training_data_citeseer/train.h5", "w") as ofile:
    ...
    with h5py.File("training_data_citeseer/test.h5", "w") as ofile:
        in_feat_dset = ofile.create_dataset(in_feat, data=feat_matrix)
        in_adj_dset = ofile.create_dataset(in_adj, data=adj_matrix)
