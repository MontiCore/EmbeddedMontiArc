...
abstract class NearestNeighborHeapSearch implements java.io.Serializable {
	...
	public static enum PartialKernelDensity implements Density, java.io.Serializable {
		...
		LOG_EXPONENTIAL {
			...
			@Override
			public double getNorm(double h, int d) {
				return logSn(d - 1) + GlobalState.Mathematics.lgamma(d);
			}
		},
		...
		LOG_GAUSSIAN {
			...
			@Override
			public double getNorm(double h, int d) {
				return 0.5 * d * GlobalState.Mathematics.LOG_2PI;
			}
		},
	...
	static double logSn(int n) {
		return GlobalState.Mathematics.LOG_2PI + logVn(n - 1);
	}
	...
	static double logVn(int n) {
		return 0.5 * n * GlobalState.Mathematics.LOG_PI - GlobalState.Mathematics.lgamma(0.5 * n + 1);
	}
