 ...
 */
public class LatencyViewEffSymbolCreator implements TagSymbolCreator {
  ...
  public void create(ASTTaggingUnit unit, Scope gs) {
    ...
    for (ASTTag element : unit.getTagBody().getTags()) {
      element.getTagElements().stream()
          .filter(t -> t.getName().equals("LatencyViewEff"))
          ...
          .forEachOrdered(v ->
              element.getScopes().stream()
                ...
                .map(s -> getGlobalScope(gs).<Symbol>resolveDownMany(
                    Joiners.DOT.join(rootCmp, ((ASTConnectorScope)s).getSource()+s.getQualifiedName().toString()),
                    SymbolKind.KIND))
                .filter(s -> !s.isEmpty())
                .map(this::checkKind)
                .filter(s -> s != null)
                .forEachOrdered(s -> s.addTag(new LatencyViewEffSymbol(
  ...
  protected ViewEffectorSymbol checkKind(Collection<Symbol> symbols) {
    ViewEffectorSymbol ret = null;
    for (Symbol symbol : symbols) {
      if (symbol.getKind().isSame(ViewEffectorSymbol.KIND)) {
        if (ret != null) {
          Log.error(String.format("0xA4095 Found more than one symbol: '%s' and '%s'",
              ret, symbol));
          return null;
        }
        ret = (ViewEffectorSymbol)symbol;
      }
    }
    if (ret == null) {
      Log.error(String.format("0xT0001 Invalid symbol kinds: %s. tagTypeName expects as symbol kind 'ViewEffectorSymbol.KIND'.",
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. LatencyViewEff expects as scope kind 'ConnectorScope'.",
