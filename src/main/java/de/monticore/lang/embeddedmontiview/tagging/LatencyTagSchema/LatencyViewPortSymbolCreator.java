 ...
 */
public class LatencyViewPortSymbolCreator implements TagSymbolCreator {
  ...
  public void create(ASTTaggingUnit unit, Scope gs) {
    ...
    for (ASTTag element : unit.getTagBody().getTags()) {
      element.getTagElements().stream()
          .filter(t -> t.getName().equals("LatencyViewPort"))
          ...
          .forEachOrdered(v ->
              element.getScopes().stream()
                ...
                .map(this::checkKind)
                .filter(s -> s != null)
                .forEachOrdered(s -> s.addTag(new LatencyViewPortSymbol(
  ...
  protected ViewPortSymbol checkKind(Collection<Symbol> symbols) {
    ...
    if (ret == null) {
      Log.error(String.format("0xT0001 Invalid symbol kinds: %s. tagTypeName expects as symbol kind 'ViewPortSymbol.KIND'.",
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. LatencyPort expects as scope kind 'NameScope'.",
