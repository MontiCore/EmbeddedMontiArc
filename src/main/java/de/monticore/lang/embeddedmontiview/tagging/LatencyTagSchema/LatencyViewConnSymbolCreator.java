 ...
 */
public class LatencyViewConnSymbolCreator implements TagSymbolCreator {
  ...
  public void create(ASTTaggingUnit unit, Scope gs) {
    ...
    for (ASTTag element : unit.getTagBody().getTags()) {
      element.getTagElements().stream()
          .filter(t -> t.getName().equals("LatencyViewConn"))
          ...
          .forEachOrdered(v ->
              element.getScopes().stream()
                ...
                .map(this::checkKind)
                .filter(s -> s != null)
                .forEachOrdered(s -> s.addTag(new LatencyViewConnSymbol(
  ...
  protected ViewConnectorSymbol checkKind(Collection<Symbol> symbols) {
    ...
    if (ret == null) {
      Log.error(String.format("0xT0001 Invalid symbol kinds: %s. tagTypeName expects as symbol kind 'ViewConnectorSymbol.KIND'.",
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. LatencyViewConn expects as scope kind 'ConnectorScope'.",
