 ...
 */
public class TagTableSymbolCreator implements TagSymbolCreator {
            ...
            element.getTagElementList().stream()
                    ...
                    .forEachOrdered(v ->
                            element.getScopeList().stream()
                                    ...
                                    .map(this::checkKind)
                                    .filter(s -> s != null)
                                    .forEachOrdered(s -> gs.addTag(s, new TagTableSymbol(v.toArray(new Double[v.size()])))));
        }
    }
    ...
    protected ExpandedComponentInstanceSymbol checkKind(Collection<Symbol> symbols) {
        ...
        for (Symbol symbol : symbols) {
            if (symbol.getKind().isSame(ExpandedComponentInstanceSymbol.KIND)) {
                if (ret != null) {
                    Log.error(String.format("0xT0027 Found more than one symbol: '%s' and '%s'",
                            ret, symbol));
                    return null;
                }
        ...
        if (ret == null) {
            Log.error(String.format("0xT0028 Invalid symbol kinds: %s. tagTypeName expects as symbol kind 'ExpandedComponentInstanceSymbol.KIND'.",
    ...
    protected boolean checkScope(ASTScope scope) {
        ...
        Log.error(String.format("0xT0029 Invalid scope kind: '%s'. PowerConsumption expects as scope kind 'NameScope'.",
