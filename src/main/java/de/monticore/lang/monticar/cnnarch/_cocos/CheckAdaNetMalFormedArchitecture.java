            ...
            for (NetworkInstructionSymbol networkInstruction : architecture.getNetworkInstructions()) {
                for (ArchitectureElementSymbol layer : networkInstruction.getBody().getElements()) {
                    if (!(layer.equals(input) || layer.equals(output))) {
                        ...
                        if (!toMany) {
                            // check if the adaNet input is the architecture input
                            // samce check with the output
                            boolean inputCheck = layer.getInputElement().get().equals(input);
                            boolean outputCheck = layer.getOutputElement().get().equals(output);
                            toMany = inputCheck & outputCheck;
                        }
                    }
                }
