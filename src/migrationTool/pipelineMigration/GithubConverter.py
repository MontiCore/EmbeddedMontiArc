...
class GithubActionConverter(Converter):
  ...
  def trigger(repo_name: str, branch: str = "master") -> str:
    ...
    """
    trigger = ""
    trigger += "\t\t\t- name: Record trigger time\n"
    trigger += "\t\t\t\tid: trigger_time\n"
    trigger += "\t\t\t\trun: echo \"time=$(date -u +'%Y-%m-%dT%H:%M:%SZ')\" >> $GITHUB_OUTPUT\n"
    ...
    trigger += "\t\t\t- name: Wait for workflow to appear\n"
    trigger += "\t\t\t\trun: sleep 20\n"
    trigger += "\t\t\t- name: Get latest run ID of Child Workflow after trigger time\n"
    trigger += "\t\t\t\tid: get_run\n"
    trigger += "\t\t\t\trun: |\n"
    ...
    trigger += ("\t\t\t\t\tRESULT=$(gh run view \"${{ steps.get_run.outputs.run_id }}\" --repo=$REPO --json conclusion "
                "--jq '.conclusion')\n")
    trigger += "\t\t\t\t\techo \"Child Workflow result: $RESULT\"\n"
    trigger += "\t\t\t\t\tif [[ \"$RESULT\" != \"success\" ]]; then\n"
    trigger += "\t\t\t\t\t  echo \"Child Workflow failed\"\n"
    trigger += "\t\t\t\t\t  exit 1\n"
    trigger += "\t\t\t\t\tfi\n"
    trigger += "\t\t\t\tenv:\n"
    trigger += "\t\t\t\t\tGH_TOKEN: ${{github.token}}\n"
