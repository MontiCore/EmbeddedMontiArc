...
import simulation._cocos.*;
import simulation._od.*;
import simulation._parser.*;
import simulation._symboltable.*;
import simulation._visitor.*;
import si._ast.*;
import si._cocos.*;
import si._od.*;
import si._parser.*;
...
public class SimulationTool {
  ...
  public static void main(String[] args) {
    ...
    Log.info("Simulation DSL Tool", SimulationTool.class.getName());
    Log.info("------------------", SimulationTool.class.getName());
    String model = args[0];
    ...
    Log.info(model + " parsed successfully!", SimulationTool.class.getName());
    ...
    //cs.handle(ast);
    //Log.info("The model contains " + cs.getCount() + " states.", SimulationTool.class.getName());
    
    // execute a pretty printer
    //PrettyPrinter pp = new PrettyPrinter();
    //pp.handle(ast);
    //Log.info("Pretty printing the parsed automaton into console:", SimulationTool.class.getName());
  ...
  public static ASTSimulation parse(String model) {
    try {
      ...
      Optional<ASTSimulation> optSimulation = parser.parse(model);
   ...
   * @return
   */
  /*public static Scope createSymbolTable(WeatherLanguage lang, ASTSimulation ast) {
    ...
    Optional<SimulationSymbolTableCreator> symbolTable = lang.getSymbolTableCreator(
   ...
   * @param ast
   */
  public static void runDefaultCoCos(ASTSimulation ast) {
    final SimulationCoCoChecker checker = new SimulationCoCoChecker();
