...
#ifndef CNNCALCULATOR_DIGITCOMBINER_TEST
#define CNNCALCULATOR_DIGITCOMBINER_TEST

//#include "catch.hpp"
#include "../cNNCalculator_digitCombiner.h"
#include <iostream>
#include <fstream>
#include <string>

namespace cNNCalculator_digitCombiner_test{
...
void test_case_cNNCalculator_DigitCombinerTest(){
    mat tmpA;
    mat tmpB;
    cNNCalculator_digitCombiner component;
    std::ofstream myfile;
    myfile.open ("cNNCalculator.DigitCombinerTest");
    myfile.precision(64);
    component.init();
            component.tens = 1.0;
            component.ones = 1.0;
            component.hundreds = 1.0;
        component.execute();

    rangeValueCheck(component.number, 111.0 , 111.0);
            myfile << "number: ";
            toFileString(myfile, component.number);
            myfile << "\n";
    myfile.close();
    std::cout << "cNNCalculator.DigitCombinerTest: success\n";
}
...
int runTest(){
    ...
    __StacktraceFile << "~Entering cNNCalculator.DigitCombinerTest" << std::endl;
    __StacktraceFile.close();
    test_case_cNNCalculator_DigitCombinerTest();
